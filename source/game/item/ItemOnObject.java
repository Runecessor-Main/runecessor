		...
		player.getEventHandler().addEvent(player, new CycleEvent<Entity>() {
			...
			public void execute(CycleEventContainer<Entity> container) {
				if (player.itemOnObjectEvent) {
					...
					switch (objectId) {
					case 6:
						if(itemId == 2) {//cannonball
							game.object.custom.Object serverObject = Server.objectManager.isServerGeneratedObject(player.getObjectX(), player.getObjectY(), player.getHeight());
