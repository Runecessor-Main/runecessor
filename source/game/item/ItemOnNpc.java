...
public class ItemOnNpc {
	...
	public static void itemOnNpc(Player player, int itemId, int npcId, int slot) {
			...
			if (ItemDefinition.getDefinitions()[itemId].note) {
				...
				if (!ItemDefinition.getDefinitions()[unNotedId].note) {
					if (ItemAssistant.hasItemAmountInInventory(player, itemId, amountToUnNote)) {
						...
						// So if you unnote 28 dark crabs with 27 inventory space, it unnotes all dark
						// crabs.
						...
						player.getDH().sendItemChat("",
								"You un-note " + amountToAdd + " " + ItemAssistant.getItemName(unNotedId) + ".",
								unNotedId, 200, 10, 0);
					}
				} else {
				...
				if (ItemDefinition.getDefinitions()[notedId].note) {
					...
					player.getDH().sendItemChat("",
							"You note " + amountToNote + " " + ItemAssistant.getItemName(itemId) + ".", itemId, 200, 10,
							0);
				} else {
		...
		if (npc.npcType == 5523) {
			if (ItemAssistant.playerHasItem(player,4273, 5, slot)) {
				player.getDH().sendStatement("Hello "+player.getPlayerName()+ " I just have Changed your Key of tokens to 1 token!");
				ItemAssistant.deleteItemFromInventory(player, 4273, 5);
				ItemAssistant.addItemToInventoryOrDrop(player, 7478, 5);
				return;
			} else if (!ItemAssistant.playerHasItem(player,4273, 5, slot)) {
				player.getDH().sendStatement("Hello "+player.getPlayerName()+ " You must have 5 key of tokens to do this!");
			     return;
			}
