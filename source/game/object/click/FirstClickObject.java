...
public class FirstClickObject {
    ...
    public static void firstClickObjectOsrs(final Player player, int objectId, int objectX, int objectY) {
        ...
        Object serverObject = Server.objectManager.isServerGeneratedObject(player.getObjectX(), player.getObjectY(), player.getHeight());
        Object globalObject = Region.getObjectForRegion(objectX, objectY, player.getHeight());
...
//			player.getSkill(TestSkill.class).perform(new MineRockAction());
        ...
        switch (objectId) {
            ...
            case 15566:
                ...
                break;
            /*
             * buged was fixed by fezo case 7170: if
             * (ItemAssistant.getFreeInventorySlots(player) == 0) { return; }
             * ItemAssistant.addItem(player, 7478,
             * ItemAssistant.getFreeInventorySlots(player)); ItemAssistant.addItem(player,
             * 7478, ItemAssistant.getFreeInventorySlots(player));
             * player.startAnimation(832); break;
             */
            case 31990:
                ...
                if (player.getY() <= 4052) {
                    if (!player.getVorkathLostItems().isEmpty()) {
                        player.setDialogueChainAndStart(new DialogueChain().statement(
                                "You need to claim your lost items from Torfinn", "before you can fight Vorkath."));
                        return;
                    }
                    try {
                        final VorkathSinglePlayerMinigame singlePlayerMinigame = VorkathSinglePlayerMinigameFactory
                                .create(player);
            ...
            case 9385:
            case 9380:
                HunterTrap trapToDismantle = HunterTrapObjectManager
                        .getSingleton().getObjects().stream().filter(t -> t.getOwner().equals(player.getPlayerName())
                                && t.objectId == objectId && t.objectX == objectX && t.objectY == objectY)
                        .findAny().orElse(null);
            ...
            case 9382:
                HunterTrapObjectManager.getSingleton().getObjects().stream()
                        .filter(t -> player.getPlayerName().equalsIgnoreCase(t.getOwner()) && t.objectId == objectId
                                && t.objectX == objectX && t.objectY == objectY)
                        .findAny().ifPresent(trap -> trap.check(player));
            ...
            case 24452:
                ...
                boolean sent = false;
                try {
                    long time = (System.currentTimeMillis() - StaffActivity.timeWeeklyStaffActivityChecked)
                            / Misc.getHoursToMilliseconds(1);
                    double days = (double) time / 24.0;
                    text = "Staff activity statistics for the last " + Misc.roundDoubleToNearestTwoDecimalPlaces(days)
                            + " days:";
                    sent = true;
                } catch (Exception e) {
                    e.printStackTrace();
                ...
                for (int index = 0; index < StaffActivity.staffActivityList.size(); index++) {
                    ...
                    text = instance.getName() + ": "
                            + Misc.roundDoubleToNearestTwoDecimalPlaces(((double) instance.getMinutesActive() / 60.0))
                            + " hours active.";
            ...
            case 11845:
                ...
                if (area != null && area.getKey() == MinigameAreaKey.FIGHT_PIT_COMBAT_AREA
                        && area.getPlayerParticipant(player) != null) {
            ...
            case 9758:
                /*
                 * if(ItemAssistant.hasItemAmountInInventory(player, 16502, 2000)) {
                 * ItemAssistant.deleteItemFromInventory(player, 16502, 2000);
                 * ItemAssistant.addItemToInventoryOrDrop(player, 16503, 1);
                 * player.getPA().announce(" "+player.getPlayerName()
                 * +" Reward X1 Easter Blood Money Box from event chest");
                 *
                 *
                 *
                 * } else { player.getDH().sendItemChat("", "Hello "+player.getPlayerName()+
                 * " Get 2k Easter key and come again!.", 16502, 200, 14, 0); return; }
                 */
                break;
            ...
            case 7142:
                if (!player.isSuperDonator()) {
                    player.getPA().sendMessage(DonatorTokenUse.getDonatorRankIcon(DonatorRankSpentData.SUPER_DONATOR)
                            + "This Donator dungeon is only for Super Donators and above. Type in ::donate");
                    player.getPA().sendMessage(DonatorTokenUse.getDonatorRankIcon(DonatorRankSpentData.SUPER_DONATOR)
                            + "Donators earn 20-35k an hour depending on their rank.");
                    return;
                }
            ...
            case 26760:
                ...
                if (player.getY() == 3945) {
                    ...
                    if (ItemAssistant.checkAndDeleteStackableFromInventory(player, ServerConstants.getMainCurrencyId(),
                            fee)) {
                        player.getPA().sendMessage("You pay " + fee + " "
                                + ServerConstants.getMainCurrencyName().toLowerCase() + " to enter.");
                        canEnter = true;
                    } else {
                        player.getDH().sendItemChat("",
                                "You need to pay a fee of " + fee + " "
                                        + ServerConstants.getMainCurrencyName().toLowerCase() + " to enter.",
            ...
            case 17385:
                ...
                } else if (player.getObjectX() == 3084 && player.getObjectY() == 9672) {
            ...
            case 14893:
            case 14894:
                Teleport.spellTeleport(player, 3078, 3486, 0, false);
                break;
            ...
            // case 8151:
            case 8174:
                // if (player.getObjectId() == 8151 && Config.ECO || player.getObjectId() ==
                // 8174 && Config.PVP) {
            ...
            case 10043:
                ...
                if (player.getObjectX() == 2911) {
                    ...
                    if (!ItemAssistant.hasItemAmountInInventory(player, 8850, 1)) {
                        player.setNpcType(2135);
                        player.getDH().sendNpcChat("You can't enter there without a rune defender!",
                                DialogueHandler.FacialAnimation.CALM_2.getAnimationId());
                        return;
                    }
            ...
            // Mining guild ladder and also Edgeville dungeon ladder to get to Wilderness
            // orb thing.
            case 1755:
                switch (player.getObjectX()) {
            ...
            /*
             * //Cave entrance to fight Jad case 11833: if (player.getObjectX() == 2437 &&
             * player.getObjectY() == 5166) { FightCaves.startFightCaves(player); } break;
             */
            ...
            // 2020 EASTER EVENT
            case 19337:
                player.getDH().sendDialogues(738);
                break;

            case 21800:
                player.startAnimation(749);
                player.getPA().movePlayer(3080, 3485, 0);
                break;
            ...
            // Ladder in a shed at west Varrock, that leads to Edgeville dungeon, at Hill
            // giants
            case 17384:
            ...
            default:
                if (Plugin.execute("first_click_object_" + objectId, player)) {
                    ...
                    PacketHandler.unUsedObject.add("Unused object: " + player.getObjectId() + ", " + player.getObjectX()
                            + ", " + player.getObjectY() + ", " + player.getHeight());
