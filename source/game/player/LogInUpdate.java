 ...
 *
 * @author Lava, created on 13-12-2013.
 */
public class LogInUpdate {
	...
	public static void update(Player player) {
		...
		/*
		 * if(ServerConstants.DOUBLE_EXP == false) {
		 * player.getPA().sendMessage("[Double Exp] Stauts: <col=FF0000>Offline"); }
		 * if(ServerConstants.DOUBLE_EXP == true) {
		 * player.getPA().sendMessage("[Double Exp] Stauts: <col=00FF00>Online"); }
		 */
		...
		player.getPA().setChatOptions(player.publicChatMode, player.privateChat, player.tradeChatMode); // Reset private																	
		...
		Server.dayCycle.refreshTimeOfDay();
...
//		 HighscoresDaily.getInstance().informHighscores(player);
					...
					if (player.musicPacketsReceived < 2) {
						if (!player.addressIp.isEmpty() && !Blacklist.floodIps.contains(player.addressIp)
							...
							Blacklist.floodBlockReason.add("[" + Misc.getDateAndTime() + "] " + player.getPlayerName()
									+ ", " + player.addressIp + ", " + player.addressUid
	...
	private static void connectionCheck(Player player) {
		...
		if (HostList.countConnections(
				player.addressIp) > (ServerConfiguration.DEBUG_MODE ? 2000 : ServerConstants.IPS_ALLOWED)) {
	...
	/**
	 * Update certain parts of the game for the player, before the player can see
	...
	private static void updateBeforeNotice(Player player) {
		...
		player.playerAssistant.sendMessage(":rights" + player.playerRights + ":"); // Tell client the player rights, to
																					// use on player name chat area
																					// crown.
