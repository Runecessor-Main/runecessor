...
public class PlayerHandler {
				...
				if (hasDisconnectRequirements(players[i])) {
					...
					if (!players[i].initialized) {
						...
						if (duplicate > 1) {
							...
							Misc.print("Logged in: "+players[i].getPlayerName());
							players[i].onAdd();
	...
	public static Optional<Player> getOptionalPlayer(String name) {
		return getPlayers().stream().filter(Objects::nonNull).filter(client -> client.getPlayerName().equalsIgnoreCase(name)).findFirst();
	}
