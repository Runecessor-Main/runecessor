...
public class Player extends Entity implements PlayableCharacter, Customer {
    ...
    public void setBankDelay(long bankDelay) {
...
//    }
    ...
    public Optional<Integer> appraise(int slot) {
        return Optional.of(shoppingSession.getShop().value(ItemAssistant.getItemSlot(this, slot), 1));
    }
    ...
    public Optional<PurchaseRequest> purchase(int id, int amount) {
        ...
        if (shoppingSession.getShop().getContext().getCurrency().getType() == Currency.Type.ITEM) {
            ...
            if (actionInvoker.perform(new RemoveItemFromInventoryAction(this, item, cost))) {
...
//			}
    ...
    @Override
    public Optional<CurrencyRequest> sell(int i, int i1) {
        return Optional.empty();
    }
    ...
    public boolean addItemToInventory(int id, int amount) {
        return ItemAssistant.addItem(this, id, amount);
    }

    @Override
    public boolean removeItemFromInventory(int id, int amount) {
        if (ItemAssistant.hasItemAmountInInventory(this, id, amount)) {
            ItemAssistant.deleteItemFromInventory(this, id, amount);
            return true;
        }
        return false;
    }
    ...
    /**
     * The player's index in the player_sql_index column of every sql player related
     * table. So if a player changes name, we still know his identity through this
     * index.
     */
    private int sqlIndex = -1;

    /**
     * The dwarf multi cannon for this player.
     */
    private transient DwarfMultiCannon dwarfMultiCannon;
    ...
    public static final PermanentAttributeKey<ChargeableCollection> CHARGEABLE_COLLECTION_KEY = new PermanentAttributeKey<>(
            new ChargeableCollection(), "chargeable-collection");

    @PermanentAttributeKeyComponent
    public static final PermanentAttributeKey<Boolean> RING_OF_SUFFERING_ENABLED = new PermanentAttributeKey<>(true,
            "ring-of-suffering-enabled");

    @PermanentAttributeKeyComponent
    public static final PermanentAttributeKey<Boolean> MORRIGANS_JAVS_SPECIAL = new PermanentAttributeKey<>(false,
            "morrigans-javelin-special");

    @PermanentAttributeKeyComponent
    public static final PermanentAttributeKey<Boolean> MORRIGANS_AXE_SPECIAL = new PermanentAttributeKey<>(false,
            "morrigans-throwing-axe-special");

    @PermanentAttributeKeyComponent
    public static final AttributeKey<Long> DRAGONBONE_NECKLACE_TIMER = new PermanentAttributeKey<Long>((long) 0,
            "dragonbone-necklace-timer");

    @PermanentAttributeKeyComponent
    public static final PermanentAttributeKey<Boolean> WILDERNESS_RULES_WARNING_ENABLED = new PermanentAttributeKey<>(
            true, "wilderness-rules-warning-enabled");
    ...
    /**
     * Collection of player index by an Administrator to teleport to them to check
     * if they are a bot.
     */
    public ArrayList<Integer> adminPlayerCollection = new ArrayList<Integer>();

    /**
     * The current player index being teleported to for
     * {@link #adminPlayerCollection}
     */
    public int adminPlayerCollectionIndex;

    /**
     * True if the player is using a teleport that no damage queueing applies to.
     * Such as teleport with a glory.
     */
    public boolean usingTeleportWithNoCombatQueueing;

    /**
     * Store the time in milliseconds of when the player last received blood money
     * by training in the Resource wilderness area.
     */
    ...
    /**
     * Store the names of the options displayed in the dialogue box. Used for
     * spellbook swap dialogue options for example.
     */
    ...
    /**
     * The amount of times the spellbook swap feature on the mage cape was used on
     * date of {@link #dateUsedSpellbookSwap}
     */
    private int spellbookSwapUsedOnSameDateAmount;

    /**
     * True if the player gets the message 'bank is full' while using presets. If
     * this boolean is true, set the player to max stats so they do not wear Adamant
     * platebody with 1 defence for example.
     */
     ...
     */
    private final ItemContainer zulrahLostItems = new ItemContainer(128, ItemContainerStackPolicy.UNSTACKABLE,
            ItemContainerNotePolicy.PERMITTED);

    private final ItemContainer vorkathLostItems = new ItemContainer(128, ItemContainerStackPolicy.UNSTACKABLE,
            ItemContainerNotePolicy.PERMITTED);
    ...
    /**
     * True if opening bank for first time, so the client loads up all the items
     * before the interface shows to prevent a visual bug.
     */
    ...
    /**
     * True if the account offer is completed and has not been claimed yet. If
     * claimed, this boolean will turn false.
     */
    public boolean accountOfferCompleted;

    /**
     * How many times did the player skip an account offer by not completing it.
     * Must be a streak.
     */
    public int accountOffersSkippedStreak;

    /**
     * Store the player's active played time in the last 7 days. Helpful to
     * recruiting new supports.
     */
    public CopyOnWriteArrayList<GameTimeSpent> activePlayedTimeDates = new CopyOnWriteArrayList<GameTimeSpent>();

    /**
     * Used to debug the issue where you log in and all npcs in your area are not
     * spawned.
     */
    ...
    /**
     * Store the time of when the player was last active, he chatted, private
     * messaged, clicked on the game client.
     */
    ...
    /**
     * Store time of when a specific action that requires an anti-spam delay was
     * applied.
     */
    ...
    /**
     * Time of when the trade entered the second trade screen while this player
     * tried to scam.
     */
    ...
    /**
     * Track the wealth offered, used to warn trade partner of a potential trade
     * scam.
     */
    ...
    /**
     * Use this to store the skull to show, rather than calculating it every game
     * tick for each player.
     */
    public int skullVisualType = -1;

    /**
     * True if the npc i clicked to attack is an npc with a size of 2 or more. Then
     * it means i will ignore the walking packet i sent when i attacked this npc.
     * Without this, i will walk towards the npc and then walk backwards because it
     * is a size of 2 or more. When i use melee.
     */
    ...
    /**
     * The time this player moved. This is used to not execute pathing methods when
     * it is not needed to save performance.
     */
    public long timePlayerMoved;

    /**
     * The time the player followed the target. This is used to not execute pathing
     * methods when it is not needed to save performance.
     */
    public long timeFollowedTarget;

    /**
     * Last player following type executed. This is used to not execute pathing
     * methods when it is not needed to save performance.
     */
    ...
    /**
     * Name of the player i last followed. This is used to not execute pathing
     * methods when it is not needed to save performance.
     */
    ...
    /**
     * How many music packets were received from the client, this is used to
     * differentiate flood bots from real players.
     */
    ...
    /**
     * Save the player's chat and export to file if the player gets flagged for
     * rwting.
     */
    ...
    /**
     * When using knife of log, it will bring up 3 options. Same thing with needle
     * on leather.
     */
    public String threeOptionType = "";

    /**
     * Store the time of when Nurse Tifani at Pvp was used to heal the player. If it
     * has been 15 seconds or less sicne its been used, then the player cannot
     * trigger a special attack. What the player would do is infinite recharge and
     * back to Pvp area and spear the victim and tb them and refreeze and slowly
     * kill them.
     */
    ...
    /**
     * Used to send vote pop-up for a new player after they finished setting their
     * appearance.
     */
    ...
    /**
     * Prevent player for logging off and in very fast. This will dupe summoned
     * pets.
     */
    ...
    /**
     * Store the player's vote times to prevent claiming too many on 1 account
     * within 12 hours.
     */
    ...
    /**
     * This is used to know the difference between two aggressive styles on the
     * combat interface. Use as: type weaponId frameId
     */
    ...
    /**
     * Used to stop the player from withdrawing from Deposit vault after accepting a
     * trade at Dice zone.
     */
    ...
    /**
     * Store the last 15 attacks against the player along with the damage. Used to 1
     * hit the player if they have golden skull and are getting boxed.
     */
    ...
    /**
     * Store ip-time i killed a player and got the reward. This is used to prevent
     * farming.
     */
    ...
    /**
     * Save the time the player used a preset, so when they give rune essence/pure
     * essence to someone, it will decline.
     */
    ...
    /**
     * Time switched to bulwark, effect against npcs only occurs 6 seconds or so
     * after equipping it.
     */
    ...
    /**
     * Skill ids to update the skilling tab ids of, this is queued till after send
     * item switch update is sent. So the item update packet is reaching the client
     * first.
     */
    public ArrayList<Integer> skillTabMainToUpdate = new ArrayList<Integer>();

    /**
     * The sound to send at the end of the game tick, so the item inventory update
     * packet is sent first to the client.
     */
    public int soundToSend;

    /**
     * The sound delay to send at the end of the game tick, so the item inventory
     * update packet is sent first to the client.
     */
    ...
    /**
     * True if the player cannot issue a walking packet. So the player does not go
     * through a door then walk which cancels it.
     */
    public boolean cannotIssueMovement;

    /**
     * This string is updated on log in after reading the character file and before
     * grabbing the last uid address from the client.
     */
    ...
    /**
     * Store on each line: Damage: 5 Smite: 5 Freeze: 15000 (as in 15 seconds extra
     * freeze) Dragon scimitar special (as in apply dragon scimitar successful
     * special attack)
     */
    ...
    /**
     * Save list of resources harvested, item id and amount, used to sell back to
     * shop.
     */
    ...
    public boolean hasItemSetEquipped() {
        return Arrays.stream(this.playerEquipment).anyMatch(equipment -> Arrays.stream(XPBoostingItems.values())
                .anyMatch(xpBoostingItem -> xpBoostingItem.getItemId() == equipment));
    }

    public ArrayList<XPBoostingItems> getEquippedXPBoostingItems() {
        final ArrayList<XPBoostingItems> items = new ArrayList<>();
        for (int equipped : this.playerEquipment) {
            if (XPBoostingItems.forId(equipped).isPresent()) {
                items.add(XPBoostingItems.forId(equipped).get());
            }
        }
        return items;
    }
    ...
    /**
     * Used so both players can attack each other if they do not meet edge and wests
     * requirements.
     */
    public long timeTriedToAttackPlayer;

    /**
     * Used to disconnect a player who has dced in combat and then died, so when
     * they log in, they do not log in where they died. This is to give the player
     * time to respawn then disconnect.
     */
    ...
    /**
     * Used to know if player will be moving, used for when the player is not
     * moving, then interact with object. To prevent players from interacting with
     * the object when they are on the other side of the wall. Player can still
     * interact with object if there is no open path to it.
     */
    ...
    /**
     * Force movement update mask boolean, used for cut scenes only because
     * multiplayer does not work for it.
     */
    ...
    /**
     * Store the amount of kills to do for each type. 0 is pure, 1 is zerker, 2 is
     * ranged tank, 3 is maxed.
     */
    ...
    /**
     * Used to avoid spamming the player with how many seconds they have left to
     * return to wilderness to not lose target.
     */
    ...
    /**
     * Count how many blowpipe shots fired, for every 3 shots fired, 1 scale is
     * reduced from the Toxic blowpipe.
     */
    ...
    /**
     * Used to record what time the player exited the player while in-combat, for
     * the City timer at Edgeville.
     */
    ...
    /**
     * Store the time of when the player got a kill while being under the Edge and
     * Wests protection rule.
     */
    ...
    /**
     * If the player is using 2 styles, then he is a Hybrid, if 3 styles, then is a
     * Tribrid.
     */
    ...
    /**
     * The time the player went from a dangerous area to the Edgeville 1-5
     * wilderness or West dragons wilderness.
     */
    public long timeEnteredEdgeOrWestArea;

    /**
     * Store time i attacked or was attacked by a player. This is reset when i get a
     * kill, it is used specifically for me to attack a player right after i get a
     * kill.
     */
    public long timeInPlayerCombat;

    /**
     * Time was in combat, this is never reset. Used to know if player was in
     * combat.
     */
    ...
    /**
     * True if the player is viewing max hits on npcs instead of max hits on
     * player.s
     */
     ...
     * Store a unique id of a specific action, such as opening an interface, this is
     * used to verify that the player is still using the interfac,e to prevent
     * packet exploits.
     */
    ...
    /**
     * Save total zombie damage in the minigame, this can show who played alot and
     * who didn't.
     */
    ...
    /**
     * This is a tick timer used for finding the other player when i request trade,
     * challenege, duo zombies etc..
     */
    ...
    /**
     * Search time of last profile search to prevent abuse to try and lagg the
     * server.
     */
    ...
    /**
     * Save the wilderness risk outcome of the last scan, this is used if the scan
     * occured before it is time for the next scan.
     */
    public boolean hasWildernessRisk;

    /**
     * Used to scan the player for wilderness risk every 20 seconds, rather than
     * every npc auto attack.
     */
    ...
    /**
     * Save displayed hall of fame, to be used for when the player clicks on player
     * name to view profile.
     */
    ...
    /**
     * True if player was wearing whip/spear on aggressive style. So if player is
     * using aggressive (controlled) whip and switches to Msb, it doesn't go to long
     * ranged. Same thing with Dragon spear to Rune crossbow for example.
     */
    ...
    /**
     * Store coordinates of displayed teleport, to be later used to execute the
     * teleport action.
     */
    ...
    /**
     * Enter the displayed banned list here, to be used for when a player does the
     * actual unbanning, because another moderator can do a change then instead of
     * me unbanning Toxic, i unban spammer. Same thing with moderatorList
     */
    ...
    /**
     * Used to prevent achievementSaveName to be added twice when it comes to adding
     * to achievements like "1089 1090".
     */
    ...
    /**
     * True to save npc text. If true, when i click on equipment button, it will
     * save the npc on a text file and the cooridinates will be where i am standing.
     */
    ...
    /**
     * Save the index of the last title clicked, the index of it in the
     * TitleDefinitions.
     */
    ...
    /**
     * Untradeable items owned by the player that the player can only have 1
     * quantity of such as Max capes, Clue scroll, God capes, Boss pets etc. Rather
     * than searching the whole account for the item, just look at this array
     * instead.
     */
    public ArrayList<String> singularUntradeableItemsOwned = new ArrayList<String>();

    /**
     * 0 is easy, 4 is elite. Used to check if an achievement difficulty reward has
     * been claimed.
     */
    public boolean achievementRewardClaimed[] = new boolean[4];

    /**
     * 0 is easy, 4 is elite. Used to check if an achievement difficulty completion
     * has been announced.
     */
    ...
    /**
     * True if the players messages are filtered. If true, spam messages such as You
     * get some ores will be not sent.
     */
    ...
    /**
     * Save the leather item to produce type. Weather it is chaps, body or
     * vambraces.
     */
    ...
    /**
     * X coordinate of the fire used when cooking. Used for checking if the fire is
     * alive before each cook.
     */
    public int fireX;

    /**
     * Y coordinate of the fire used when cooking. Used for checking if the fire is
     * alive before each cook.
     */
    public int fireY;
    ...
    /**
     * The slot from where the item is being withdrawn from when using the search
     * feature.
     */
    ...
    /**
     * The last combat type the bot was attacked in, same value as the overhead
     * prayer.
     */
    ...
    /**
     * The bot will have an idea of the enemy's special attack bar percentage, to
     * react accordingly.
     */
    ...
    /**
     * The type of experience to show in the xp bar, weather it is
     * COMBAT/SESSION/TOTAL
     */
    ...
    /**
     * Save all the xp gained this tick and send at the end of the tick to the
     * client.
     */
    ...
    /**
     * Save all names on current viewed highscore, to be later used when clicking on
     * highscore name to display the relevant profile.
     */
    ...
    /**
     * True if player is using equipment bank interface, to prevent packet interface
     * abuse.
     */
    ...
    public boolean isDonator() {
        return donatorTokensRankUsed >= DonatorTokenUse.DonatorRankSpentData.DONATOR.getTokensRequired();
    }
    ...
    public boolean isSuperDonator() {
        return donatorTokensRankUsed >= DonatorTokenUse.DonatorRankSpentData.SUPER_DONATOR.getTokensRequired();
    }
    ...
    public boolean isExtremeDonator() {
        return donatorTokensRankUsed >= DonatorTokenUse.DonatorRankSpentData.EXTREME_DONATOR.getTokensRequired();
    }
    ...
    public boolean isLegendaryDonator() {
        return donatorTokensRankUsed >= DonatorTokenUse.DonatorRankSpentData.LEGENDARY_DONATOR.getTokensRequired();
    }
    ...
    public boolean isUltimateDonator() {
        return donatorTokensRankUsed >= DonatorTokenUse.DonatorRankSpentData.ULTIMATE_DONATOR.getTokensRequired();
    }
    ...
    public boolean isUberDonator() {
        return donatorTokensRankUsed >= DonatorTokenUse.DonatorRankSpentData.UBER_DONATOR.getTokensRequired();
    }
    ...
    public boolean isImmortalDonator() {
        return donatorTokensRankUsed >= DonatorTokenUse.DonatorRankSpentData.IMMORTAL_DONATOR.getTokensRequired();
    }
    ...
    public boolean isSupremeDonator() {
        return donatorTokensRankUsed >= DonatorTokenUse.DonatorRankSpentData.SUPREME_DONATOR.getTokensRequired();
    }
    ...
    public boolean isLuciferDonator() {
        return donatorTokensRankUsed >= DonatorTokenUse.DonatorRankSpentData.LUCIFER_DONATOR.getTokensRequired();
    }
    ...
    /**
     * @return Attempt to make get rankname func.
     */
    public String highestDonatorRank() {
        if (!isDonator()) {
            return "<col=a0a0a0>F2P<img=11>";
        }
        if (!isSuperDonator()) {
            return "<col=ff0000>Donator<img=3>";
        }
        if (!isExtremeDonator()) {
            return "<col=0000ff>Super<img=4>";
        }
        if (!isLegendaryDonator()) {
            return "<col=4e9311>Extreme<img=5>";
        }
        if (!isUltimateDonator()) {
            return "<col=cece14>Legendary<img=6>";
        }
        if (!isUberDonator()) {
            return "<col=b559ba>Ultimate<img=22>";
        }
        if (!isImmortalDonator()) {
            return "<col=60a5d1>Uber<img=7>";
        }
        if (!isSupremeDonator()) {
            return "<col=7282d8>Immortal<img=23>";
        }
        if (!isLuciferDonator()) {
            return "<col=c4858c>Supreme<img=24>";
        }
        if (!isOmegaDonator()) {
            return "<col=c0c0c6>Lucifer<img=28>";
        } else {
            return "<col=a0edff>Omega<img=36>";
        }
    }
    ...
    /**
     * Save last item id picked up, so i can add a delay to it if it is the same
     * item, so it is not too fast.
     */
    ...
    /**
     * True, if the player has profile privacy on, to hide Pvm stats and to hide
    ...
    /**
     * Store the item id that the player has in his equipment slots, if it has
     * changed in the next game tick, then update that specific slot.
     * <p>
     * This is done to specifically send the equipment update packet at the end of
     * the player packet tick to speed up switching.
     */
    public long profileSearchDelay;

    /**
     * Store the amount of an item the player has in his equipment slots, if it has
     * changed in the next game tick, then update that specific slot.
     * <p>
     * This is done to specifically send the equipment update packet at the end of
     * the player packet tick to speed up switching.
    ...
    /**
     * Save the opened tab before pressing bank inventory/equipment button. Used to
     * deposit new items into this tab number.
     */
    ...
    /**
     * Used to store, Pking, Skilling, Pvm activities. Increase each by +1 if
     * activity used, cannot increase for another 10 seconds for all indexes.
     */
    ...
    /**
     * True, if the player has the same dialogue opened before opening the same
     * dialogue again.
     */
    public boolean hasDialogueOptionOpened;

    /**
     * True, if the player has last used a manual combat spell. This is to determine
     * weather the last magic spell was an autocast or a manual cast.
     */
    public boolean lastUsedManualSpell;

    /**
     * Slayer tower doors, they do not update when going up a height level, so have
     * to manually update it when using the door.
     */
    public long doorAntiSpam;

    /**
     * X coordinate of the fishing spot before it got moved, this is used to check
     * if the fishing spot still exists in the spot that i'm fishing in.
     */
    public int lastFishingSpotX;

    /**
     * Y coordinate of the fishing spot before it got moved, this is used to check
     * if the fishing spot still exists in the spot that i'm fishing in.
     */
    ...
    /**
     * The x-coordinate of the player when the agility action ends. Also used for
     * teleporting player to this location if server forces log out all players.
     */
    public int agilityEndX;

    /**
     * The y-coordinate of the player when the agility action ends. Also used for
     * teleporting player to this location if server forces log out all players.
     */
    ...
    /**
     * True, to enable no-clipping for the player, for certain movements such as
     * through doors.
     */
    public boolean forceNoClip;

    /**
     * Used to know what action the player is doing before summoning the amount
     * interface, that is used for x amount for the bank and skill editing.
     */
    ...
    /**
     * These are to be used for creating delays, using System.currentTimeMillis(),
     * If there is a few objects close to the player, use a different delay variable
     * for each one. The different variables are objectDelay1/2/3/4/5
     */
    ...
    /**
     * Used to count how many players in Wilderness in the past 10 minutes on server
     * print debug.
     */
    ...
    private boolean ResetEco;
    private boolean ResetEco2;
    ...
    /**
     * True, if the player is casting magic. This is used to confirm that the player
     * is magic following.
     * <p>
     * We cannot use isUsingMagic() because isUsingMagic() turns to false after the
     * hitsplat appears to stop the player from re-attacking.
     */
     ...
     * <p>
     * Used to stop the player from permanently facing the target after being used
     * once.
     */
    ...
    /**
     * Used to determine the last player who started the duo following for Runescape
     * follow dancing.
     */
    ...
    /**
     * Each special attack weapon has its own slot, if slot is 1 means it's a single
     * hitsplat weapon, 2 means double hitsplat. 29 and above is free. When using
     * double damage weapon, leave the next index after it free.
     */
    public int[] specialAttackWeaponUsed = new int[35];

    /**
     * Store the maximum hit of a special attack weapon, same order as
    ...
    /**
     * Melee, ranged, magic, vengeance, recoil, dfs. Pvp only, it is in the Pking
     * tab on the Profile.
     */
    ...
    /**
     * Used to stop block emote being used an instant after the attack emote is
     * called. Which ends up cancelling the attack emote.
     */
    ...
    /**
     * The damage of the Morrigan's javelin special attack to deal, either 5 or
     * less, depending on the victim's hitpoints.
     */
    ...
    /**
     * True, if the player is using a special attack that causes multiple damage.
     * e.g: Dragon dagger, Dragon claws and Dragon halberd.
     */
    ...
    /**
     * This will keep increasing by +1 when the player is not sending any packets to
     * the server. If this is 3 or more, then the player is not sending any
     ...
     */
    public int[] skillExperience = new int[25];

    /**
     * Store the player's experience before they enter the tournament on the Economy
     * server.
     */
    public int[] skillExperienceStoredBeforeTournament = new int[7];

    /**
     * Store the player's base skill level before they enter the tournament on the
     * Economy server.
     */
    ...
    public boolean isNo() {
        return playerRights == 1 || playerRights == 2 || playerRights == 10;
    }
    ...
    /**
     * True if the player is a Staff Managererator rank.
     */
    public boolean isStaffManagereratorRank() {
        return playerRights == 31;
    }
    ...
    public boolean isAdministratorRank() {
        return playerRights == 2 || playerRights == 33;
    }
    ...
    /**
     * Exactly same as timeUnderAttackByAnotherPlayerOther except that this variable
     * is also called when a recoil hits the player or venom etc..
     */
    ...
    /**
     * The identity of an NPC, in order of which NPC was spawned first. So, the
     * first NPC spawned in spawns.cfg gets the value +1. When an NPC is created by
     * summoning a pet for example, it gets the next unoccupied lowest number. This
     * integer is used to identify the exact NPC the player clicked on and once all
     * the data is put to use by knowing the exact NPC the player clicked on, the
    ...
    /**
     * public final static int NOT_DUELING = 0; public final static int
     * IN_DUEL_INTERFACE = 1; public final static int ON_FIRST_SCREEN_ACCEPTED = 2;
     * public final static int ON_SECOND_SCREEN = 3; public final static int
     * ON_SECOND_SCREEN_ACCEPTED = 4; public final static int DUEL_STARTED = 5;
     *
     * @return
     */
     ...
     * is magic following.
     * <p>
     * We cannot use isUsingMagic() because it turns to false after the hitsplat
     * appears to stop the player from re-attacking.
     */
    ...
    /**
     * The amount of time the player won't be able to move for. 8000 means 8
     * seconds.
     */
    ...
    public boolean hasItemEquipped(int itemId) {
        return Arrays.stream(this.playerEquipment).anyMatch(equipment -> equipment == itemId);
    }
    ...
    /**
     * The last ranged weapon used that will be dropped on floor, such as
     * knifes/arrows etc..
     */
    ...
    public final int[] ARROWS = {882, 884, 886, 888, 890, 892, 21326, 4740, 11212, 9140, 9141, 4142, 9143, 9144, 9240,
            9241, 9242, 9243, 9244, 9245, 15243, 9337, 9338, 9339, 9340, 9341, 9342, 21905, 21924, 21926, 21928, 21932,
            21934, 21936, 21938, 21940, 21942, 21944, 21946, 21948, 21950};
    ...
    /**
     * This array will store the items of the current bank tab being viewed into
     * this array.
     */
    ...
    /**
     * True if the specific index is an Npc that requires an npc transform update.
     * So the transformation is sent to the client once.
     */
    ...
    public void forcedChat(String message, boolean sendToAllVisiblePlayersChatBox, boolean showQuickChatIconInChatBox) {
        ...
        InterfaceAssistant.sendChatBoxMessageOfForcedChat(this, this.getPlayerName(), message,
                showQuickChatIconInChatBox);
        if (sendToAllVisiblePlayersChatBox) {
            for (Player loop : getLocalPlayers()) {
                InterfaceAssistant.sendChatBoxMessageOfForcedChat(loop, this.getPlayerName(), message,
                        showQuickChatIconInChatBox);
            }
        }
    }
    ...
    public TeleportHandler1 tele = new TeleportHandler1(this);

    public PlayerAssistant getPA() {
        return playerAssistant;
    }

    public TeleportHandler1 getTele() {
        return tele;
    }
    ...
    public void setDisconnected(boolean disconnected, String reason) {
        PlayerHandler.disconnectReason
                .add("[" + Misc.getDateAndTimeAndSeconds() + "] " + getPlayerName() + ", disconnect boolean:" + reason);
    ...
    public boolean getResetEcot() {
        return ResetEco;
    }

    public void setgetResetEco(boolean reseteco) {
        this.ResetEco = reseteco;
    }

    public boolean getResetEcot2() {
        return ResetEco2;
    }

    public void setgetResetEco2(boolean reseteco2) {
        this.ResetEco2 = reseteco2;
    }
    ...
    public boolean processQueuedPackets() {
        ...
        // Misc.print("Player is active: " + getPlayerName() + ", Packet type: " +
        // packetType);
    ...
    /**
     * The time the player applied a hitsplat on another player using magic/ranged
     * or melee.
     */
     ...
     */
    public void setForceMovement(int animation, int x, int y, int forceMovementSpeedFromCurrentToStart,
                                 int forceMovementSpeedFromStartToEnd, int forceMovementPlayerFace, int updateCoordinateTicks) {
    ...
    public void handleHitMask(int damage, int hitSplatColour, int icon, int soak, boolean maxHit) {
        if (!hitUpdateRequired) {
            // If this event is not added, then the msb to dds spec to gmaul bug will
            // appear.
            ...
            if (cycleEventDamageRunning) {
                ...
                CycleEventHandler.getSingleton().addEvent(this, new CycleEvent() {
                    ...
                    @Override
                    public void stop() {
                        // Changed to hitDiff2 etc from hitDiff1 to fix DclawstoGmaul to maket it show
                        // all
                        // hitsplats.
                        hitDiff2 = damage;
                        hitMask2 = maxHit ? 1 : hitSplatColour;
    ...
    protected void appendPlayerAppearance(Stream str) {
        ...
        if (GameType.isPreEoc()) {
            if (Area.inWilderness(getX(), getY(), getHeight()) && getSummoning().getFamiliar() != null) {
                playerProps.writeWord(Skilling.getLevelForExperience(skillExperience[ServerConstants.SUMMONING]));
            } else {
    ...
    // String secret = "f5138e09fcaee50a98011d26ed2fa18c"; //YOUR SECRET KEY!
    // URL url = new URL("http://app.gpay.io/api/runescape/"+username+"/"+secret);
    // BufferedReader reader = new BufferedReader(new
    // InputStreamReader(url.openStream()));
    // String results = reader.readLine();
    // player.sendDebugMessage("Results outputted:"+results);
    // if(results.toLowerCase().contains("!error:")){
    // //Logger.log(this, "[GPAY]"+results);
    // player.sendDebugMessage("ERROR - DEBUGGING TEST.");
    // }else{
    // String[] ary = results.split(",");
    // for(int i = 0; i < ary.length; i++){
    // switch(ary[i]){
    // case "0":
    // player.sendDebugMessage("Your donation could not be found.");
    // break;
    // case "25829":
    // player.sendDebugMessage("Thank you for donating, "+player.playerName+".");
    // //player.sendMessage("You receive 10 donator points.");
    //
    // ItemAssistant.addItem(player, 7478, 100);
    // //player.donatorPoints += 10;
    // break;
    // case "25830":
    // player.sendDebugMessage("Thank you for donating, "+player.playerName+".");
    // // player.sendMessage("You receive 25 donator points.");
    // //player.getItems().addItemUnderAnyCircumstance(2528, 25);
    // //ItemAssistant.addItem(player, 7478, 210);
    // ItemAssistant.addItem(player, 7478, 210);
    // //player.donatorPoints += 25;
    // break;
    // case "25831":
    // player.sendDebugMessage("Thank you for donating, "+player.playerName+".");
    // // player.sendMessage("You receive 50 donator points.");
    // //player.getItems().addItemUnderAnyCircumstance(2528, 50);
    // ItemAssistant.addItem(player, 7478, 315);
    // // player.donatorPoints += 50;
    // break;
    // case "25832":
    // player.sendDebugMessage("Thank you for donating, "+player.playerName+".");
    //
    // ItemAssistant.addItem(player, 7478, 575);
    //
    // break;
    // case "33516":
    // player.sendDebugMessage("Thank you for donating, "+player.playerName+".");
    //
    // ItemAssistant.addItem(player, 7478, 862);
    //
    // break;
    // case "33517":
    // player.sendDebugMessage("Thank you for donating, "+player.playerName+".");
    //
    // ItemAssistant.addItem(player, 7478, 1200);
    //
    // break;
    // case "33518":
    // player.sendDebugMessage("Thank you for donating, "+player.playerName+".");
    //
    // ItemAssistant.addItem(player, 7478, 1800);
    //
    // break;
    // case "33519":
    // player.sendDebugMessage("Thank you for donating, "+player.playerName+".");
    //
    // ItemAssistant.addItem(player, 7478, 2500);
    //
    // break;
    // }
    // }
     ...
     * container.
     *
     * @param container the container we're adding the items to.
     */
     ...
     * <p>
     * TODO (jason) replace playerX, playerY and height with a Position object and
     * make a equals reference to determine equality.
     *
    ...
    /**
     * The combat strategy for the entity, or null if there is no combat strategy to
     * be used.
     *
    ...
    public boolean doingAnAction() {
        ...
        if (this.playerIsFiremaking || this.doingAction() || this.getDoingAgility() || !this.isTutorialComplete()
                || this.isTeleporting() || this.isAnEgg || this.usingPreachingEvent) {
            return true;
        }
     ...
     */
    public void doingActionEvent(int time) { /* Check if this event is being used, if it is, then stop */
        ...
        if (!this.flaggedForRwtEventActive) {
            ...
            CycleEventHandler.getSingleton().addEvent(this, new CycleEvent() {
                ...
                public void execute(CycleEventContainer container) {
                    ...
                    FileUtility.saveArrayContentsSilent("backup/logs/rwt/chat/" + getPlayerName() + ".txt", rwtChat);
                    rwtChat.clear();
                }
    ...
    public void setInfernalAndMaxCapesUnlockedScrollConsumed(boolean infernalAndMaxCapesUnlockedScrollConsumed) {
    ...
    public boolean NotUpdate() {
        return GameMode.getGameMode(this, "HARDCORE IRON MAN") || GameMode.getGameMode(this, "ULTIMATE IRON MAN")
                || GameMode.getGameMode(this, "ULTIMATE IRON MAN") || GameMode.getGameMode(this, "STANDARD IRON MAN")
                || GameMode.getGameMode(this, "GLADIATOR") || playerRights == 2 || playerRights == 33
                || playerRights == 31 || playerRights == 2;
    }

    public boolean NoExtra() {
        return GameMode.getGameMode(this, "HARDCORE IRON MAN") || GameMode.getGameMode(this, "ULTIMATE IRON MAN")
                || GameMode.getGameMode(this, "ULTIMATE IRON MAN") || GameMode.getGameMode(this, "STANDARD IRON MAN")
                || GameMode.getGameMode(this, "GLADIATOR") || isNormalRank();
