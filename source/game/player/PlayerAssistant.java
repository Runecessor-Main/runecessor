...
public class PlayerAssistant {
	...
	private final int[] backupItems = new int[ServerConstants.BANK_SIZE];
	private final int[] backupItemsN = new int[ServerConstants.BANK_SIZE];
	...
	public void sendItemContainer(org.menaphos.entity.impl.item.container.ItemContainer container, int interfaceId) {
		...
		for (ItemSlot itemSlot:container.getReadOnlyContents()) {
			int itemId = itemSlot == null ? 0 : itemSlot.getId();

			int amount = itemSlot == null ? 0 : itemSlot.getAmount().value();
