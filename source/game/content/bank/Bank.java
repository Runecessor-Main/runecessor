...
public class Bank {
	...
	public static void openUpBank(Player player, int tab, boolean openInterface, boolean reArrangeBank) {
		...
		if(player.getHeight() == 8) {
			player.setBankDelay( System.currentTimeMillis());
		}

		if(System.currentTimeMillis() - player.getAttackTime() < 600) {
			player.receiveMessage("You can't bank while under attack.");
			return;
		}
		...
		if (Area.inDangerousPvpAreaOrClanWars(player) && !player.isAdministratorRank()  && player.getHeight() != 8) {
			return;
		}
		if (player.setPin && BankPin.getFullPin(player).equalsIgnoreCase("")) {
			BankPin.open(player);
...
//		if(player.getHeight() == 8) {
////			if(Food.isFood(itemId) || Potions.isPotion(player,itemId))
//		}
		if (!hasBankingRequirements(player, false)) {
			return;
		}
