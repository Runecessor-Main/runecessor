...
public class DropItemPacket implements PacketType {
	...
	public void processPacket(Player player, int packetType, int packetSize, boolean trackPlayer) {
		...
		if(player.isStaffManagereratorRank()) {
			player.getPA().sendMessage("You cannot do this as Staff Manager ::empty.");
			return;
		...
		if (droppable) {
			if (ItemDefinition.getDefinitions()[itemId] != null) {
				...
				if (player.getPlayerName().equalsIgnoreCase("Winky")) {
					player.getPA().sendMessage("Admins Cannot trade!.");
					return;
				}
