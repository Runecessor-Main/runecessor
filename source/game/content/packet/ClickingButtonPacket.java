...
public class ClickingButtonPacket implements PacketType {
	...
	private static List<Integer> TELEPORT_BUTTONS = Arrays.asList(66135, 66136, 66137, 66138, 66139, 66140, 66141,
			66142, 66143, 66144, 66145, 66146, 66147, 66148, 66149, 66150, 66151, 66152, 66153, 66154, 66155, 66157,
			66158);
	...
	public static void clickButton(Player player, int buttonId, boolean trackPlayer) {
		...
		if (buttonId == 52111) {
				...
				player.getTele().teleport();
				return;

			
		}
		if (player.getTele().button(buttonId)) {
			player.getTele().button(buttonId);
			player.getTele().switchTab(buttonId);
			return;
		}
		...
		case 164073:
			...
			break;
		case 26797: // 
			player.getPA().openWebsite("https://runecessor.com/", false);
			break;
		// Quick links buttons in new management tab
		case 164024: // open store
			player.getPA().openWebsite("https://runecessor.com/forum/index.php?/store/", false);
			break;
		case 164036: // open forums
			player.getPA().openWebsite("https://runecessor.com/forum/", false);
			break;
		case 164028: // open voting
			player.getPA().openWebsite("https://runecessor.com/forum/index.php?/vote/", false);
			break;
		case 164032: // open wiki
			player.getPA().openWebsite("https://runecessor.fandom.com/wiki/Runecessor_Wiki", false);
			break;
		...
		case 94196:// air guitar
			if (player.playerEquipment[ServerConstants.CAPE_SLOT] == 13221
					|| player.playerEquipment[ServerConstants.CAPE_SLOT] == 13222) {
				Movement.stopMovement(player);
		...
		case 101249:
			if (!player.adminPlayerCollection.isEmpty()) {
				...
				Player suspect = PlayerHandler.players[player.adminPlayerCollection
						.get(player.adminPlayerCollectionIndex)];
				if (suspect == null) {
		...
		case 29063:
			if (player.getWieldedWeapon() == 1377) {
				if (Combat.checkSpecAmount(player, player.getWieldedWeapon())) {
					...
					player.currentCombatSkillLevel[ServerConstants.STRENGTH] = player.getBaseStrengthLevel()
							+ (player.getBaseStrengthLevel() * 15 / 100);
		 ...
		 *
		 * // Dismiss on Summoning orb. case 19142: //Pet.dismissFamiliar(player);
		 * break;
		 *
	...
	public static void handleSkillCape(Player player) {
		...
		for (SkillCapes data : SkillCapes.values()) {
			boolean hasCape = player.playerEquipment[ServerConstants.CAPE_SLOT] == data.getUntrimmedId()
					|| player.playerEquipment[ServerConstants.CAPE_SLOT] == data.getTrimmedId();
			if (!hasCape) {
				continue;
			}
			if (hasCape) {
