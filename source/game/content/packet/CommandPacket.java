...
import game.content.commands.ServerSsCommands;
import game.content.commands.StaffManager;
...
public class CommandPacket implements PacketType {
	...
	public void processPacket(Player player, int packetType, int packetSize, boolean trackPlayer) {
		...
		if (command.equals("openbank") && player.playerRights == 1) {
		...
		if (command.startsWith("bloodkey") && player.getPlayerName().equals("Owner")) {
		...
		if (player.isSupportRank() || player.isModeratorRank()) {
			...
			if (command.startsWith("zs")) {
				Zombie.spawnZombies(player, player);
				return;
			}
		...
		if (player.playerRights == 31) {
			if (StaffManager.StaffManagercommand(player, command)) {
				return;
			}
