 ...
 * Teleport interface.
 *
 * @author Lava, created on 05-09-2016.
 */
public class TeleportInterface {
	...
	public static void teleportStartUp() {
		 ...
		 * skilling.add("Mining ; 3023 9739 0"); skilling.add("Smithing ; 2469 3436 0");
		 * skilling.add("Fishing ; 2552 3563 0"); skilling.add("Cooking ; 2998 3906 0");
		 * skilling.add("Woodcutting ; 2998 3906 0");
		 * skilling.add("Agility ; 2998 3906 0"); skilling.add("Farming ; 2816 3462 0");
		...
		add(wilderness, "Yveltal Dragon @red@(Wilderness 19) ; 3303 3669 0", GameType.OSRS);
		...
		add(wilderness, "Revenant Caves @red@(Wilderness 40) ; 3127 3833 0", GameType.ANY);// TODO - replace with
																							// pre-eoc rev caves
		...
		add(wilderness, "Tekton @red@(Wilderness 50) ; 3307 3925 0", GameType.OSRS);
		...
		// add(minigame,"Zombie survival ; 3659 3516 0", GameType.OSRS_ECO);

		// minigame.add("Zombie survival ; 3659 3516 0");

		donator.add("<img=3>Old DonatorZone ; 2192 3251 0");
		add(donator, "<img=4>DungeonZone ; 1748 5325 0", GameType.OSRS);
		donator.add("<img=5>ExtremeZone ; 2530 2724 0");
		donator.add("<img=3>NewZone ; 2982 2783 0");
	...
	public static boolean isTeleportInterfaceButton(Player player, int buttonId) {
		if (buttonId >= 77023 && buttonId <= 77119) {
	...
	private static void teleportAction(Player player, int buttonId) {
		...
		String coordinate = player.currentTeleports.get(index)
				.substring(player.currentTeleports.get(index).indexOf(";") + 2);
		...
		if (name.contains("DonatorZone") && !player.isDonator()) {
			player.getPA().sendMessage(DonatorTokenUse.getDonatorRankIcon(DonatorRankSpentData.DONATOR)
					+ "The Donator zone is only for Donators. Type in ::donate");
			return;
		}
		if (name.contains("NewZone") && !player.isDonator()) {

			player.getPA().sendMessage(DonatorTokenUse.getDonatorRankIcon(DonatorRankSpentData.DONATOR)
					+ "The Donator zone is only for Donators. Type in ::donate");
			return;
		}
		if (name.contains("ExtremeZone") && !player.isExtremeDonator()) {
			player.getPA().sendMessage(DonatorTokenUse.getDonatorRankIcon(DonatorRankSpentData.EXTREME_DONATOR)
					+ "This Donator dungeon is only for Extreme Donators and above. Type in ::donate");
			return;
		}
		if (name.contains("DungeonZone") && !player.isSuperDonator()) {
