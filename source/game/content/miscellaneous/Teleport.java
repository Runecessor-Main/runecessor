 ...
 *
 * @author Lava, created on 29-11-2013.
 */
...
public class Teleport {
	...
	private static boolean canTeleport(Player player, String teleportType, int x, int y, int height) {
		...
		if (teleportType.contains("GLORY") || teleportType.contains("SEED_POD")) {
			...
			if (!(player.getPetId() == 11212) &&!Area.inCityPvpArea(player) && Area.inDangerousPvpArea(player) && player.getWildernessLevel() > 20 && !teleportType.contains("OBELISK") && !teleportType.contains(
