 ...
 * Death stages.
 *
 * @author Lava, created on 25-11-2013.
 */
public class Death {
    ...
    private static void hardcoreIronManDeath(Player victim) {
        if (GameMode.getGameMode(victim, "HARDCORE IRON MAN") && !victim.isUsingFightCaves()) {
    ...
    private static void deathRespawnArea(Player victim, Player killer) {
        ...
        if (victim.getHeight() == 20) {
            ...
            victim.getPA().movePlayer(3093, 3478, 0);
        } else {
    ...
    private static boolean diedInSafeArea(Player killer, Player victim) {
        if (victim.isStaffManagereratorRank() || victim.isAdministratorRank() || victim.isUsingFightCaves() || victim.getDuelStatus() == 5) {
        ...
        if (killer != null) {
            if (killer.isStaffManagereratorRank() || killer.getDuelStatus() == 6) {
                return true;
            }
        }
        if (killer != null) {
    ...
    public static void victimLoot(Player victim, Player killer, int itemId, int amount, boolean victimBot, boolean killerExists) {
        if (ItemAssistant.isItemToUntradeableShopOnDeath(itemId)) {
            ...
            // Charged ethereum bracelet.
                ...
                if (victim.getActiveBossInstance() != null) {
//                    ShopFactory.getShop(1).add(itemId,amount);
                    Server.itemHandler
                            .createGroundItem(killer, itemId, 3087, 3493, 0, amount, false, 0, true, victim.getPlayerName(), killer.getPlayerName(),
