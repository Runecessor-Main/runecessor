...
public class Combat {
	...
	public static void attackApplied(Player attacker, Player victim, String type, boolean specialAttack) {
		...
		victim.setAttackTime(System.currentTimeMillis());
	...
	public static int wyvernProtection(Player player, boolean npc) {
		...
		if (player.playerEquipment[ServerConstants.SHIELD_SLOT] == 9731 // Mind shield
		    || player.playerEquipment[ServerConstants.SHIELD_SLOT] == 11284 // Dragonfire shield
		    		|| player.playerEquipment[ServerConstants.SHIELD_SLOT] == 2890 // Dragonfire shield
		    || player.playerEquipment[ServerConstants.SHIELD_SLOT] == 21633 // Ancient wyvern shield
		    || player.playerEquipment[ServerConstants.SHIELD_SLOT] == 22002) // Dragonfire ward
		{
			resistance++;
	...
	public static void castVengeance(Player player) {
		...
		if (System.currentTimeMillis() - player.lastVeng > 30000) {
			if (MagicData.requiredRunes(player, 53, "CHECK REQUIREMENT")) {
				...
				Skilling.addSkillExperience(player, 1, ServerConstants.MAGIC, false);
				player.gfx100(726);
			} else {
	...
	public static boolean wearingFullPernix(Player player) {
		return  player.playerEquipment[ServerConstants.HEAD_SLOT] == 4928 && player.playerEquipment[ServerConstants.LEG_SLOT] == 4946 && player.playerEquipment[ServerConstants.BODY_SLOT] == 2030;
	...
	public static boolean checkSpecAmount(Player player, int weapon) {
		...
		if (GameType.isPreEoc()) {
			...
			if (itemName.equals("Armadyl crossbow") || itemName.equals("Vorktah crossbow")) {
	...
	public static void addCombatExperience(Player player, int attackType, int damage) {
		addCombatExperience(player, attackType, damage, false);
	}
	 ...
	 */
	public static void addCombatExperience(Player player, int attackType, int damage, boolean cannon) {
		...
		if(!cannon) {
			...
			if (player.getSpellId() >= 0) {
				if (damage > 0) {
					Skilling.addSkillExperience(player, (int) ((damage * 1.3)), Skill.HITPOINTS.getId(), false); // Hitpoints experience.
				}
			} else {
				Skilling.addSkillExperience(player, (int) ((damage * 1.3)), Skill.HITPOINTS.getId(), false); // Hitpoints experience.
			}
	...
	public static int getRangeProjectileGFX(Player player) {
		...
		// Armadyl crossbow.
		if (ItemAssistant.getItemName(player.getWieldedWeapon()).equals("Vorktah crossbow")&&ItemAssistant.getItemName(player.getWieldedWeapon()).equals("Armadyl crossbow") && player.armadylCrossbowSpecial) {
			return 301;
		}
		// Dragon crossbow.
