 ...
 *
 * @author Lava, Edited heavily on 02-11-2015 to make the Xp drops appear before
 * hitsplat.
 */
public class CombatNpc {
    ...
    public static void applyPoisonOnNpc(Player player, Npc npc, int poisonDamage) {
        ...
        npc.getEventHandler().addEvent(npc, new CycleEvent<Entity>() {
            ...
            public void stop() {
                if (player == null) {
                    CombatNpc.applyHitSplatOnNpc(null, npc, poisonDamage, ServerConstants.POISON_HITSPLAT_COLOUR,
                            ServerConstants.NO_ICON, 1);
                } else {
                    npc.underAttack = true;
                    CombatNpc.applyHitSplatOnNpc(player, npc, poisonDamage, ServerConstants.POISON_HITSPLAT_COLOUR,
                            ServerConstants.NO_ICON, 1);
                }
            }
        }, 1);
        ...
        npc.setPoisonCycleEvent(npc.getEventHandler().addEvent(npc, new CycleEvent<Entity>() {
            ...
            public void execute(CycleEventContainer container) {
                ...
                if (npc.poisonTicksUntillDamage == 0) {
                    if (player == null) {
                        CombatNpc.applyHitSplatOnNpc(null, npc, damage, ServerConstants.POISON_HITSPLAT_COLOUR,
                                ServerConstants.NO_ICON, 1);
                    } else {
                        CombatNpc.applyHitSplatOnNpc(player, npc, damage, ServerConstants.POISON_HITSPLAT_COLOUR,
                                ServerConstants.NO_ICON, 1);
                    }
            ...
            public void stop() {
                if (player == null) {
                    if (npc.venomDamage == 6) {
                        CombatNpc.applyHitSplatOnNpc(null, npc, npc.venomDamage, ServerConstants.VENOM_HITSPLAT_COLOUR,
                                ServerConstants.NO_ICON, 1);
                    } else if (npc.venomDamage >= 20) {
                        CombatNpc.applyHitSplatOnNpc(null, npc, 20, ServerConstants.VENOM_HITSPLAT_COLOUR,
                                ServerConstants.NO_ICON, 1);
                    } else {
                        CombatNpc.applyHitSplatOnNpc(null, npc, npc.venomDamage, ServerConstants.VENOM_HITSPLAT_COLOUR,
                                ServerConstants.NO_ICON, 1);
        ...
        CycleEventHandler.getSingleton().addEvent(npc, new CycleEvent() {
            ...
            public void execute(CycleEventContainer container) {
                ...
                if (npc.venomTicksUntillDamage == 0) {
                    if (player == null) {
                        if (npc.venomDamage == 6) {
                            CombatNpc.applyHitSplatOnNpc(null, npc, npc.venomDamage,
                                    ServerConstants.VENOM_HITSPLAT_COLOUR, ServerConstants.NO_ICON, 1);
                        } else if (npc.venomDamage >= 20) {
                            CombatNpc.applyHitSplatOnNpc(null, npc, 20, ServerConstants.VENOM_HITSPLAT_COLOUR,
                                    ServerConstants.NO_ICON, 1);
                        } else {
                        ...
                        if (npc.venomDamage == 6) {
                            CombatNpc.applyHitSplatOnNpc(player, npc, npc.venomDamage,
                                    ServerConstants.VENOM_HITSPLAT_COLOUR, ServerConstants.NO_ICON, 1);
                        } else if (npc.venomDamage >= 20) {
                            CombatNpc.applyHitSplatOnNpc(player, npc, 20, ServerConstants.VENOM_HITSPLAT_COLOUR,
                                    ServerConstants.NO_ICON, 1);
                        } else {
                            CombatNpc.applyHitSplatOnNpc(player, npc, npc.venomDamage,
                                    ServerConstants.VENOM_HITSPLAT_COLOUR, ServerConstants.NO_ICON, 1);
    ...
    public static void calculateMagicDamageOnNpc(Player attacker, Npc npc) {
        ...
        if (NpcHandler.isBattleMageNpc(npc.npcType)
                && CombatConstants.MAGIC_SPELLS[attacker.getSpellId()][0] == 12871) {
        ...
        boolean task = false;
        boolean salveAmuletBoost = false;
        if (attacker.getEquippedAmulet(12017)
                || attacker.getEquippedAmulet(12018)) {
            salveAmuletBoost = true;
            for (int undead : ServerConstants.UNDEAD_NPCS_OSRS) {
                if (npc.npcType == undead) {
                    switch (attacker.playerEquipment[ServerConstants.AMULET_SLOT]) {
                        case 12017:
                            maximumDamage += maximumDamage * 0.15;
                            accuracy += accuracy * 0.15;
                            break;
                        case 12018:
                            maximumDamage += maximumDamage * 0.20;
        ...
        if (attacker.getSlayerTask() != null) {
            task = SlayerSkill.isSlayerTask(attacker.getSlayerTask().getAssignment(), npc.npcType);
            if (task) {
                if (SlayerSkill.isWearingImbuedSlayerHelm(attacker) && !salveAmuletBoost) {
                    maximumDamage *= 1.15;
                    accuracy *= 1.15;
                }
            }
        }
        if (GameType.isOsrs()) { // Thammaron's sceptre, 100% boosted accuracy & 25% bonus damage vs wilderness
            // npcs (or max hit dummy in this case)
            if (attacker.getWieldedWeapon() == 22555 && Area.inDangerousPvpArea(attacker)
                    || attacker.getWieldedWeapon() == 22555 && npc.npcType == 1337) {
                accuracy *= 2;
                maximumDamage *= 1.25;
        ...
        if (attacker.getPetId() == 8025) {
            maximumDamage *= 2.1;
        }
        if (attacker.getPetId() == 11187) {
            maximumDamage *= 3.0;
        }
        // Max hit dummy.
        if (npc.npcType == 1337) {
        ...
        if (Misc.random(npc.npcType == 2267 ? (NpcDefinition.getDefinitions()[npc.npcType].meleeDefence / 2)
                : NpcDefinition.getMagicDefence(npc.npcType)) > 10
                + Misc.random(MagicFormula.getMagicAttackAdvantage(attacker)) + bonusAttack
                && npc.npcType != 11006) {
            damage = 0;
            magicFailed = true;
        ...
        if (Misc.random((NpcDefinition.getDefinitions()[npc.npcType].meleeDefence / 6)) > 10
                + Misc.random(MagicFormula.getMagicAttackAdvantage(attacker)) + bonusAttack && npc.npcType == 11006) {
            damage = 0;
            magicFailed = true;
        }
    ...
    private static void appendMultiBarrageNPC(Player attacker, int npcId, boolean splashed) {
        if (NpcHandler.npcs[npcId] != null) {
            ...
            if (checkMultiBarrageReqsNPC(npcId)) {
                ...
                if (Misc.random(NpcDefinition.getMagicDefence(NpcHandler.npcs[npcId].npcType)) < (10
                        + Misc.random(MagicFormula.getMagicAttackAdvantage(attacker)))) {
                    if (Combat.getEndGfxHeight(attacker,
                            CombatConstants.MAGIC_SPELLS[attacker.getOldSpellId()][0]) == 100) {
                    ...
                    CombatNpc.applyHitSplatOnNpc(attacker, n, damage, ServerConstants.NORMAL_HITSPLAT_COLOUR,
                            ServerConstants.MAGIC_ICON, 1);
    ...
    public static void calculateRangedDamageOnNpc(Player attacker, Npc npc, int damageType) {
                ...
                if (npc.npcType == undead) {
                    switch (attacker.playerEquipment[ServerConstants.AMULET_SLOT]) {
                        case 12017:
                            ...
                            rangedAttack += rangedAttack * 0.15;
                            break;
                        case 12018:
                            maximum += maximum * 0.20;
                            rangedAttack += rangedAttack * 0.20;
                            break;
                    }
                    break;
                }
        ...
        if (attacker.getWieldedWeapon() == 21012
                && NpcDefinition.getDefinitions()[npc.npcType].name.toLowerCase().contains("dragon")) {
            rangedAttack *= 1.3;
            maximum *= 1.3;
        }
        ...
        if (Combat.wearingFullPernix(attacker)) {
        ...
        // Twisted bow.
        if (attacker.getWieldedWeapon() == 16052 || attacker.getWieldedWeapon() == 16056
                || attacker.getWieldedWeapon() == 16282) {
        ...
        // Opal bolts (e)
        if (Misc.hasPercentageChance(5) && (attacker.playerEquipment[ServerConstants.ARROW_SLOT] == 9236
                || attacker.playerEquipment[ServerConstants.ARROW_SLOT] == 21932)) {

            if (RangedData.hasCrossBowEquipped(attacker)) {
                if (attacker.playerEquipment[ServerConstants.ARROW_SLOT] <= RangedData.getHighestBolt(attacker,
                        attacker.getWieldedWeapon())) {
        ...
        // Ruby bolts (e)
        if (Misc.hasPercentageChance(6) && (attacker.playerEquipment[ServerConstants.ARROW_SLOT] == 9242
                || attacker.playerEquipment[ServerConstants.ARROW_SLOT] == 21944) && damage > 0) {
            if (RangedData.hasCrossBowEquipped(attacker)) {
                if (attacker.playerEquipment[ServerConstants.ARROW_SLOT] <= RangedData.getHighestBolt(attacker,
                        attacker.getWieldedWeapon())) {
                    if (attacker.getCurrentCombatSkillLevel(
                            ServerConstants.HITPOINTS) > (attacker.getBaseHitPointsLevel() / 10)) {
                        npc.gfx0(754);
        ...
        if (damage > 0 && Misc.random(9) == 1 && (attacker.playerEquipment[ServerConstants.ARROW_SLOT] == 9244
                || attacker.playerEquipment[ServerConstants.ARROW_SLOT] == 21948)) {
        ...
        // Onyx bolt (e)
        else if ((attacker.playerEquipment[ServerConstants.ARROW_SLOT] == 9245
                || attacker.playerEquipment[ServerConstants.ARROW_SLOT] == 21950) && Misc.hasPercentageChance(11)) {
            if (RangedData.hasCrossBowEquipped(attacker)) {
                if (attacker.playerEquipment[ServerConstants.ARROW_SLOT] <= RangedData.getHighestBolt(attacker,
                        attacker.getWieldedWeapon())) {
                    damage *= 1.2;
    ...
    public static void calculateMeleeDamageOnNpc(Player attacker, Npc npc, int damageType, boolean clawsFormula) {
        ...
        boolean task = false;
        boolean salveAmuletBoost = false;
        if (attacker.getEquippedAmulet(4081)
                || attacker.getEquippedAmulet(10588)
                || attacker.getEquippedAmulet(12018)) {
            salveAmuletBoost = true;
            for (int undead : ServerConstants.UNDEAD_NPCS_OSRS) {
                if (npc.npcType == undead) {
        ...
        if (attacker.getSlayerTask() != null) {
            ...
            if (task) {
                for (int index = 0; index < ServerConstants.getSlayerHelms().length; index++) {
                    ...
                    if (ItemAssistant.hasItemEquipped(attacker, helmId) && !salveAmuletBoost) {
                        maximumDamage *= 1.17;
                        accuracy *= 1.17;
                    }
                }
        ...
        if (GameType.isOsrs()) { // Viggora's chainmace, 50% boosted accuracy & damage vs wilderness npcs (or max
            // hit dummy in this case)
            if (attacker.getWieldedWeapon() == 22545 && Area.inDangerousPvpArea(attacker)
                    || attacker.getWieldedWeapon() == 22545 && npc.npcType == 1337) {
                maximumDamage *= 1.5;
                accuracy *= 1.5;
                // attacker.getPA().sendMessage("" + maximumDamage + ", " + accuracy);
            }
        ...
        if (attacker.getPetId() == 11027) {
            maximumDamage *= 1.6;
            accuracy *= 1.6;
        }

        if (attacker.getPetId() == 11023) {
            maximumDamage *= 1.3;
            accuracy *= 1.3;
        }
    ...
    public static void setEngageWithMagicVariables(Player attacker) {
        ...
        if (GameType.isPreEoc()) {
            if (attacker.getWieldedWeapon() == 22_494 && attacker.getSpellId() == -1
                    && (attacker.getAutocastId() == -1 || attacker.getAutocastId() == AutoCast.POLYPORE_AUTOCAST)) {
    ...
    private static int tormentedDemonShield(Player attacker, int damage, int npcType, boolean shield) {
        ...
        /*
         * if (shield) { attacker.getPA().
         * sendMessage("Your damage has been reduced due to the Tormented Demon's shield."
         * ); damage *= 0.7; }
         */
        return damage;
    }
    ...
    public static void attackNpc(Player attacker, Npc npc) {
        ...
        if (attacker.getWieldedWeapon() == 16356 && attacker.currentCombatSkillLevel[ServerConstants.HITPOINTS] < 70 && Misc.hasOneOutOf(5)) {
            attacker.currentCombatSkillLevel[ServerConstants.HITPOINTS] += 15;
            attacker.playerAssistant.sendMessage("Vorktah crossbow give you some heals.");
            attacker.gfx0(444);
        ...
        if (npc.isDead() || npc.maximumHitPoints <= 0) {
            ...
            // This will stop the player from turning to npc after it died when i just
            // killed it.
        ...
        if (npc.npcType == 319 && attacker.getY() == 4394) {
            attacker.playerAssistant.sendMessage("no no not work try another location no safe spot here!.");
            Movement.stopMovement(attacker);
            attacker.turnPlayerTo(npc.getX(), npc.getY());
            Combat.resetPlayerAttack(attacker);
            return;
        ...
        if (npc.npcType == 319 && attacker.getY() == 4393) {
            attacker.playerAssistant.sendMessage("no no not work try another location no safe spot here!.");
            Movement.stopMovement(attacker);
            attacker.turnPlayerTo(npc.getX(), npc.getY());
            Combat.resetPlayerAttack(attacker);
        ...
        if (npc.npcType == 319 && attacker.getY() == 4376) {
            attacker.playerAssistant.sendMessage("no no not work try another location no safe spot here!.");
            Movement.stopMovement(attacker);
            attacker.turnPlayerTo(npc.getX(), npc.getY());
            Combat.resetPlayerAttack(attacker);
        ...
        if (npc.npcType == 319 && attacker.getY() == 4392) {
            attacker.playerAssistant.sendMessage("no no not work try another location no safe spot here!.");
            Movement.stopMovement(attacker);
            attacker.turnPlayerTo(npc.getX(), npc.getY());
            Combat.resetPlayerAttack(attacker);
        ...
        if (npc.npcType == 319 && attacker.getY() == 4391) {
            attacker.playerAssistant.sendMessage("no no not work try another location no safe spot here!.");
            Movement.stopMovement(attacker);
            attacker.turnPlayerTo(npc.getX(), npc.getY());
            Combat.resetPlayerAttack(attacker);
        ...
        if (npc.npcType == 319 && attacker.getY() == 4378) {
            attacker.playerAssistant.sendMessage("no no not work try another location no safe spot here!.");
            Movement.stopMovement(attacker);
            attacker.turnPlayerTo(npc.getX(), npc.getY());
            Combat.resetPlayerAttack(attacker);
        ...
        if (npc.npcType == 319 && attacker.getY() == 4373) {
            attacker.playerAssistant.sendMessage("no no not work try another location no safe spot here!.");
            Movement.stopMovement(attacker);
            attacker.turnPlayerTo(npc.getX(), npc.getY());
            Combat.resetPlayerAttack(attacker);
        ...
        if (npc.npcType == 319 && attacker.getY() == 4374) {
            attacker.playerAssistant.sendMessage("no no not work try another location no safe spot here!.");
            Movement.stopMovement(attacker);
            attacker.turnPlayerTo(npc.getX(), npc.getY());
            Combat.resetPlayerAttack(attacker);
        ...
        if (npc.npcType == 319 && attacker.getY() == 4375) {
            attacker.playerAssistant.sendMessage("no no not work try another location no safe spot here!.");
            Movement.stopMovement(attacker);
            attacker.turnPlayerTo(npc.getX(), npc.getY());
            Combat.resetPlayerAttack(attacker);
        ...
        if (npc.npcType == 7860 && attacker.getY() == 5358) {
            attacker.playerAssistant.sendMessage("no no not work try another location no safe spot here!.");
            Movement.stopMovement(attacker);
            attacker.turnPlayerTo(npc.getX(), npc.getY());
            Combat.resetPlayerAttack(attacker);
        ...
        if (npc.npcType == 7860 && attacker.getX() == 2284) {
            attacker.playerAssistant.sendMessage("no no not work try another location no safe spot here!.");
            Movement.stopMovement(attacker);
            attacker.turnPlayerTo(npc.getX(), npc.getY());
            Combat.resetPlayerAttack(attacker);
        ...
        if (npc.npcType == 7860 && attacker.getY() == 5357) {
            attacker.playerAssistant.sendMessage("no no not work try another location no safe spot here!.");
            Movement.stopMovement(attacker);
            attacker.turnPlayerTo(npc.getX(), npc.getY());
            Combat.resetPlayerAttack(attacker);
        ...
        if (npc.npcType == 7860 && attacker.getX() == 2285) {
            attacker.playerAssistant.sendMessage("no no not work try another location no safe spot here!.");
            Movement.stopMovement(attacker);
            attacker.turnPlayerTo(npc.getX(), npc.getY());
            Combat.resetPlayerAttack(attacker);
        ...
        if (npc.npcType == 7860 && attacker.getX() == 2275) {
            attacker.playerAssistant.sendMessage("no no not work try another location no safe spot here!.");
            Movement.stopMovement(attacker);
            attacker.turnPlayerTo(npc.getX(), npc.getY());
            Combat.resetPlayerAttack(attacker);
        ...
        if (npc.npcType == 1337) {
            ...
            /*
             * if (RandomEvent.isBannedFromSkilling(attacker)) { return; }
             */
        }

        if (!Area.npcInMulti(npc, attacker.getX(), attacker.getY()) && npc.underAttackBy > 0
                && npc.underAttackBy != attacker.getPlayerId() && !Area.inMulti(npcX, npcY) && npc.npcType != 7286
                && npc.npcType != 1337) {
            Movement.stopMovement(attacker);
            attacker.turnPlayerTo(npc.getX(), npc.getY());
        ...
        if (!Area.npcInMulti(npc, attacker.getX(), attacker.getY())
                && Combat.wasUnderAttackByAnotherPlayer(attacker, 5000) && npc.npcType != 7286) {
            Movement.stopMovement(attacker);
            attacker.turnPlayerTo(npc.getX(), npc.getY());
        ...
        if ((attacker.getUnderAttackBy() > 0
                || attacker.getNpcIndexAttackingPlayer() > 0 && Combat.wasAttackedByNpc(attacker))
                && attacker.getNpcIndexAttackingPlayer() != npc.npcIndex
                && !Area.npcInMulti(npc, attacker.getX(), attacker.getY())) {
            if (npc.npcType != 7286) {
                Movement.stopMovement(attacker);
                attacker.turnPlayerTo(npc.getX(), npc.getY());
                attacker.playerAssistant.sendMessage("I am already under attack.");
        ...
        if (taskData != null) {
            if (attacker.baseSkillLevel[ServerConstants.SLAYER] < taskData.getLevelReq()
                    && taskData.isLevelRequired()) {
                attacker.playerAssistant
                        .sendMessage("You need " + taskData.getLevelReq() + " slayer to attack this monster.");
                Movement.stopMovement(attacker);
                attacker.turnPlayerTo(npc.getX(), npc.getY());
                Combat.resetPlayerAttack(attacker);
                return;
        ...
        if (npc.npcType == 7273 && !attacker.isSuperDonator()) {
            attacker.playerAssistant
                    .sendMessage("<img=4>You need to be a Super donator to attack Brutal blue dragons.");
            Combat.resetPlayerAttack(attacker);
            Movement.stopMovement(attacker);
            attacker.turnPlayerTo(npc.getX(), npc.getY());
            return;
        ...
        if (npc.npcType == 7274 && !attacker.isExtremeDonator()) {
            attacker.playerAssistant
                    .sendMessage("<img=5>You need to be an Extreme donator to attack Brutal red dragons.");
            Combat.resetPlayerAttack(attacker);
            Movement.stopMovement(attacker);
            attacker.turnPlayerTo(npc.getX(), npc.getY());
            return;
        ...
        if (npc.npcType == 7275 && !attacker.isLegendaryDonator()) {
            attacker.playerAssistant
                    .sendMessage("<img=6>You need to be a Legendary donator to attack Brutal black dragons.");
            Combat.resetPlayerAttack(attacker);
            Movement.stopMovement(attacker);
            attacker.turnPlayerTo(npc.getX(), npc.getY());
            return;
         ...
         * attacker.playerAssistant.
         * sendMessage("You may only attack Skotizo with 126 combat.");
         * Combat.resetPlayerAttack(attacker); Movement.stopMovement(attacker);
         * attacker.turnPlayerTo(npc.getX(), npc.getY()); return; }
         */
        ...
        /*
         * if (Combat.hasGraniteMaulEquipped(attacker) &&
         * attacker.graniteMaulSpecialAttackClicks > 0 &&
         * attacker.getSpecialAttackAmount() >= 5) { return; }
         */
        ...
        if (attacker.getAttackTimer() <= 0) {
            ...
            if (attacker.playerAssistant.withInDistance(attacker.getX(), attacker.getY(), npcX, npcY,
                    CombatConstants.getAttackDistance(attacker)) && (usingBow || usingMagic)) {
                Movement.stopMovement(attacker);
            }
            if (CombatNpc.isArmadylNpc(npc.npcIndex) && !usingCross && !usingBow && !usingMagic
                    && !Combat.usingCrystalBow(attacker) && !isWieldingRangedWeaponWithNoArrowSlotRequirement) {
                attacker.playerAssistant.sendMessage("You can only use range against this.");
                Combat.resetPlayerAttack(attacker);
                return;
            }

            if (!attacker.playerAssistant.withInDistance(attacker.getX(), attacker.getY(), npcX, npcY, 2)
                    && MeleeData.usingHalberd(attacker) && !isWieldingRangedWeaponWithNoArrowSlotRequirement
                    && !usingBow && !usingMagic
                    || !attacker.playerAssistant.withInDistance(attacker.getX(), attacker.getY(), npcX, npcY,
                    CombatConstants.getAttackDistance(attacker))
                    && isWieldingRangedWeaponWithNoArrowSlotRequirement && !usingBow && !usingMagic
                    || !attacker.playerAssistant.withInDistance(attacker.getX(), attacker.getY(), npcX, npcY,
                    CombatConstants.getAttackDistance(attacker)) && usingBow) {
                return;
            }

            if (!attacker.playerAssistant.withInDistance(attacker.getX(), attacker.getY(), npcX, npcY,
                    CombatConstants.MAGIC_FOLLOW_DISTANCE) && usingMagic) {
                return;
            }
            ...
            if (!attacker.playerAssistant.withInDistance(attacker.getX(), attacker.getY(), npcX, npcY,
                    NpcDefinition.getDefinitions()[npc.npcType].size)
                    && (!isWieldingRangedWeaponWithNoArrowSlotRequirement && !MeleeData.usingHalberd(attacker)
                    && !usingBow && !usingMagic)) {
                return;
            }
            ...
            if (usingBow || usingMagic || isWieldingRangedWeaponWithNoArrowSlotRequirement
                    || (attacker.playerAssistant.withInDistance(attacker.getX(), attacker.getY(), npcX, npcY, 2)
                    && MeleeData.usingHalberd(attacker))) {
                Movement.stopMovement(attacker);
            }
            if (attacker.getSpellId() >= 0) {
                if (!Combat.checkMagicRequirementsForNpcCombatAndMagicOnFloorItemPacket(attacker,
                        attacker.getSpellId())) {
                    Movement.stopMovement(attacker);
                    attacker.resetNpcIdentityAttacking();
                    attacker.turnPlayerTo(npc.getX(), npc.getY());
            ...
            if (attacker.isMeleeFollow() && attacker.getX() != npc.getX() && attacker.getY() != npc.getY()
                    && Follow.isBigNpc(npc.npcType) == 0) {
            ...
            if (!Follow.isSpecialNpcPathing(npc.npcType)) {
                ...
                if (npc.npcType != 1337) {
                    if (attacker.isMeleeFollow()) {
                        if (!Region.isStraightPathUnblocked(attacker.getX(), attacker.getY(), npc.getVisualX(),
                                npc.getVisualY(), attacker.getHeight(), 1, 1, false)) {
                            return;
                        }
                    } else {
                        if (!Region.isStraightPathUnblockedProjectiles(attacker.getX(), attacker.getY(),
                                npc.getVisualX(), npc.getVisualY(), attacker.getHeight(), 1, 1, true)) {
                            return;
                        }
                    }
                }
            }
            EntityAttackType attackType = usingBow || usingCross || isWieldingRangedWeaponWithNoArrowSlotRequirement
                    ? EntityAttackType.RANGED
                    : usingMagic && attacker.getSpellId() > 0 ? EntityAttackType.MAGIC : EntityAttackType.MELEE;

            if (strategy != null) {
                if (!strategy.canBeAttackedByType(attacker, npc, attackType)) {
                    attacker.getPA().sendMessage(String.format("This npc cannot be attacked by %s attacks.",
                            attackType.name().toLowerCase().replace("_", " ")));
                    Combat.resetPlayerAttack(attacker);
                    return;
                }
            }
            ...
            // Attacking begins.
            attacker.setAttackTimer(Combat.getAttackTimerCount(attacker,
                    ItemAssistant.getItemName(attacker.getWieldedWeapon()).toLowerCase()));
            ...
            if (attacker.isUsingSpecial() && !usingMagic) {
                if (Combat.checkSpecAmount(attacker, attacker.getWieldedWeapon())) {
                    ...
                    if (attacker.rangedSpecialAttackOnNpc) {
                        ...
                        if (ItemDefinition.getDefinitions()[attacker.getWieldedWeapon()].name.toLowerCase()
                                .contains("dark bow")
                                || ItemDefinition.getDefinitions()[attacker.getWieldedWeapon()].name.toLowerCase()
            ...
            if (!usingBow && !usingMagic && !isWieldingRangedWeaponWithNoArrowSlotRequirement) {
                attacker.setHitDelay(Combat.getHitDelay(attacker,
                        ItemAssistant.getItemName(attacker.getWieldedWeapon()).toLowerCase()));
                attacker.setProjectileStage(0);
                attacker.setOldNpcIndex(npc.npcIndex);
                calculateMeleeDamageOnNpc(attacker, npc, 1, false);

                attacker.startAnimation(MeleeData.getWeaponAnimation(attacker,
                        ItemAssistant.getItemName(attacker.getWieldedWeapon()).toLowerCase()));
            }
            if (usingBow && !isWieldingRangedWeaponWithNoArrowSlotRequirement && !usingMagic || usingCross) { // range
                // hit
                // delay
                if (usingCross) {
            ...
            } else if (isWieldingRangedWeaponWithNoArrowSlotRequirement && !usingMagic && !usingBow) { // knives, darts,
                // etc hit delay
                attacker.setDroppedRangedItemUsed(attacker.getWieldedWeapon());
                RangedAmmoUsed.deleteAmmo(attacker);
                attacker.gfx100(Combat.getRangeStartGFX(attacker));
            ...
            if (usingMagic && attacker.getSpellId() >= 0) { // magic hit delay
                ...
                if (CombatConstants.MAGIC_SPELLS[attacker.getSpellId()][4] > 0) {
                    attacker.getPA().createPlayersProjectile(pX, pY, offX, offY, 50, 78 + (distance * 25),
                            CombatConstants.MAGIC_SPELLS[attacker.getSpellId()][4], Combat.getStartHeight(attacker),
    ...
    public static void applyHitSplatOnNPC(final Player attacker, final Npc npc) { // npc hit delay
        if (npc != null) {
            ...
            if (attacker.getProjectileStage() == 0) { // melee hit damage
                ...
                if ((attacker.getWieldedWeapon() == 5698 || attacker.getWieldedWeapon() == 13271)
                        && Misc.hasPercentageChance(30)) {
                ...
                if (defenderStrategy != null) {
                    defenderStrategy.onDamageTaken(attacker, npc, attacker.meleeFirstDamage,
                            ServerConstants.MELEE_ICON);

                    if (attacker.getMultipleDamageSpecialAttack()) {
                        defenderStrategy.onDamageTaken(attacker, npc, attacker.meleeSecondDamage,
                                ServerConstants.MELEE_ICON);
                    }
                }
            }
            ...
            if (!attacker.isUsingMagic() && attacker.getProjectileStage() > 0) { // range hit damage
                ...
                // chinchompa
                boolean chinchompa = attacker.getWieldedWeapon() == 10033 || attacker.getWieldedWeapon() == 10034
                        || attacker.getWieldedWeapon() == 11959;
                ...
                if (attacker.isMagicBowSpecialAttack() || attacker.isUsingDarkBowSpecialAttack()
                        || attacker.isUsingDarkBowNormalAttack()) {
                    CombatNpc.applyHitSplatOnNpc(attacker, npc, damage2, ServerConstants.NORMAL_HITSPLAT_COLOUR,
                            ServerConstants.RANGED_ICON, 2);
                    if (defenderStrategy != null) {
                        defenderStrategy.onDamageTaken(attacker, npc, damage, ServerConstants.RANGED_ICON);
                    }
                } else if (chinchompa) {
                    ...
                    for (Npc local : attacker.getLocalNpcs()) {
                        ...
                        if (local.distanceTo(npc.getX(), npc.getY()) <= 1) {
                            ...
                            CombatNpc.applyHitSplatOnNpc(attacker, local, attacker.rangedFirstDamage,
                                    ServerConstants.NORMAL_HITSPLAT_COLOUR, ServerConstants.RANGED_ICON, 1);
             ...
             */
                ...
                if ((attacker.getWieldedWeapon() == 12904 || attacker.getWieldedWeapon() == 16209
                        || attacker.getWieldedWeapon() == 16272) && Misc.hasPercentageChance(25)) {
                    if (Venom.ENABLE_VENOM) {
                        applyVenomOnNpc(attacker, npc);
                    } else {
                        applyPoisonOnNpc(attacker, npc, 10);
                ...
                if (Area.inMulti(npc.getX(), npc.getY()) && multiMagicSpell(attacker) && damage > 0) {
                    ...
                    for (int j = 0; j < NpcHandler.npcs.length; j++) {
                        if (NpcHandler.npcs[j] != null) {
                            ...
                            int nX = NpcHandler.npcs[j].getX(), nY = NpcHandler.npcs[j].getY(), pX = npc.getX(),
                                    pY = npc.getY();
                            if ((nX - pX == -1 || nX - pX == 0 || nX - pX == 1)
                                    && (nY - pY == -1 || nY - pY == 0 || nY - pY == 1) && npc.npcIndex != j) {
    ...
    public static void fireProjectileNpc(Player player) {
        if (player.getOldNpcIndex() > 0) {
            if (NpcHandler.npcs[player.getOldNpcIndex()] != null) {
                ...
                player.getPA().createPlayersProjectile(pX, pY, offX, offY, 50, Combat.getProjectileSpeed(player),
                        Combat.getRangeProjectileGFX(player), Combat.getProjectileStartHeight(player), 31,
                        player.getOldNpcIndex() + 1, Combat.getStartDelay(player), Combat.getProjectileSlope(player));
                if (Combat.usingDbow(player)) {
                    player.getPA().createPlayersProjectile2(pX, pY, offX, offY, 50,
                            Combat.getProjectileSpeed(player) - 20, Combat.getRangeProjectileGFX(player), 43, 31,
                            player.getOldNpcIndex() + 1, Combat.getStartDelay(player),
                            Combat.getProjectileSlope(player), player.getHeight());
                }
            }
        }
    }
    ...
    public static void applyHitSplatOnNpc(Player player, Npc npc, int damage, int mask, int icon, int hitsplatIndex) {
        ...
        if (player != null) {
            ...
            if (player.getDragonClawsSpecialAttack()) {
                // Max hit dummy
                if (npc.npcType != 1337) {
                    SpecialAttackTracker.storeDragonClawsDamage(player, hitsplatIndex == 1 ? damage : -1,
                            hitsplatIndex == 2 ? damage : -1, hitsplatIndex == 3 ? damage : -1,
    ...
    public static void handleDfsNPC(Player player) {
        if (player.getNpcIdAttacking() > 0) {
            ...
            if (!player.playerAssistant.withInDistance(player.getX(), player.getY(), npc.getX(), npc.getY(),
                    NpcDefinition.getDefinitions()[npc.npcType].size)) {
                return;
            }
            if (System.currentTimeMillis() - player.dfsDelay > 120000) {
                if (player.getNpcIdAttacking() > 0 && npc != null) {
                    OverlayTimer.sendOverlayTimer(player,
                            ServerConstants.OVERLAY_TIMER_SPRITE_DRAGON_FIRE_SHIELD_SPECIAL, 120);
                    final int damage = Misc.random(25);
                    player.startAnimation(6696);
                    player.gfx0(1165);
                    applyHitSplatOnNpc(player, npc, damage, ServerConstants.NORMAL_HITSPLAT_COLOUR,
                            ServerConstants.NO_ICON, 1);
                    ...
                    player.getPA().createPlayersProjectile2(pX, pY, offX, offY, 50, 75, 1166, 37, 37,
                            -player.getOldNpcIndex() - 1, 53, 16, player.getHeight());
                    if (npc.isDead() == true) {
                        return;
                    }
