...
public class PirateBossCombatStrategy extends NpcCombatStrategy {

    private final Queue<Attack> attackQueue = new LinkedList<>();

    public PirateBossCombatStrategy() {

    }

    private class BasicMeleeAttack extends Attack {

        public BasicMeleeAttack() {
            super(null, 4177, -1, 0, 30, 5, ServerConstants.MELEE_ICON, -1);
        }

        @Override
        public void attack(Player target, Npc attacker) {
            DamageQueue.add(new Damage(target,attacker , ServerConstants.MELEE_ICON, this.getHitDelay(), this.getMaxHit(), -1));
        }
    }
