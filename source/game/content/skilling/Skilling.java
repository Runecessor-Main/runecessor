...
public class Skilling {
   ...
   /* public static void addTokens(Player player, int amount, int skill) {
    ...
    public static void addSkillExperience(Player player, int experience, int skill, boolean usingXpLamp) {
        int originalExperience = experience;
        if (!usingXpLamp) {
            ...
            if (player.hasItemSetEquipped()) {
                double modifier = player.getEquippedXPBoostingItems().stream()
                        .filter(xpBoostingItem -> skill == xpBoostingItem.getSkill().getId())
                        .mapToDouble(XPBoostingItems::getModifier)
                        .sum();
