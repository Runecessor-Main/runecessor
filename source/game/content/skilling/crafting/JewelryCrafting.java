...
public class JewelryCrafting {
    ...
    private static void mouldJewelry(final Player player, int jewelryIndex) {
        ...
        final boolean ringBling = ((itemId == 7121 || itemId == 11866) && player.getPlayerDetails().getUnlocksList().contains(SlayerUnlocks.RING_BLING.ordinal()));
        if ((itemId != 7121 && itemId != 11866) || ringBling) {
        ...
        } else if ((itemId == 7121 || itemId == 11866) && !player.getPlayerDetails().getUnlocksList().contains(SlayerUnlocks.RING_BLING.ordinal())) {
            player.receiveMessage("You need to unlock Ring Bling before you can craft these.");
        }
    }
