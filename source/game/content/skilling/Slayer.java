 ...
 * Slayer skill.
 *
 * @author Lava, created on 02-01-2015. Modified by Owain on 11-09-17
 */
public class Slayer {
	...
	public static int getTaskResetCost() {
		return GameType.isOsrsEco() ? 150000 : 1000;
	}
			...
			if (player.slayerTaskNpcAmount <= 0) {
				...
				if (GameType.isOsrsPvp()) {
					...
					if (player.isSupremeDonator()) {
						...
						points += 200;
						ItemAssistant.addItemToInventoryOrDrop(player,13307 , 800);
					} else if (player.isUberDonator()) {
						points += 150;
						ItemAssistant.addItemToInventoryOrDrop(player,13307 , 700);
					} else if (player.isUltimateDonator()) {
						ItemAssistant.addItemToInventoryOrDrop(player,13307 , 500);
						points += 100;
					} else if (player.isLegendaryDonator()) {
						points += 50;
						ItemAssistant.addItemToInventoryOrDrop(player,13307 , 300);
					} else if (player.isExtremeDonator()) {
						points += 20;
						ItemAssistant.addItemToInventoryOrDrop(player,13307 , 200);
	...
	public static String getTaskNPC(Player player) {
		if (player.slayerTaskNpcType <= 0) {
			return "No task.";
		}
