...
public class ShopAssistant {
	...
	public void priceCheckItemToSell(Player player, int itemId) {
		...
		if(player.getShoppingSession() != null) {
			player.receiveMessage("The shop will buy this for: " + NumberFormat.getInstance().format(player.appraise(itemId).orElse(0)) + " " + ItemAssistant.getItemName(Integer.parseInt(player.getShoppingSession().getShop().getContext().getCurrency().getId())));
			return;
		}
	...
	public void sellItemToShop(Player player, int itemId, int itemSlot, int amount) {
		...
		if (player.getHeight() == 20) {
			player.playerAssistant.sendMessage("You can sell it back in this event just ::empty or drop it!.");
			return;
		}
