...
public class DialogueHandler {
	...
	public void sendDialogues(int dialogue) {
		...
		switch (dialogue) {
			...
			case 198:
				sendStatement("You need 2178 total level to claim the Max cape.");
				break;
			...
			case 256:
				...
				break;
			case 257:
				sendOption("Donator shop", "Tokens TradingPost", "What is inside Mystery boxes?", "Donate now for alot of special rewards!", "Next");
				break;
			...
			case 260:
				sendOption("Claim achievement rewards", "Community event shop", "Open site");
				break;
			...
			case 562:
				sendOption("Learn how to Play", "Skip");
				break;

			case 563:
				sendNpcChat("You can buy armour, weapons, food etc from the stores", "spread around this area",
				            "@red@You can come here with the market tablet or to the ge by ::shops", FacialAnimation.DEFAULT.getAnimationId());
				player.nextDialogue = 564;
				player.getPA().movePlayer(3080, 3509, 0);
				break;

			case 564:
				sendNpcChat("You can begin to make money here by thieving the stalls.", "@red@Beware: @bla@ Auto-clicking is against the rules.", "Also Thieve with global NPC's & Stalls.", FacialAnimation.DEFAULT.getAnimationId());
				player.nextDialogue = 565;
				player.getPA().movePlayer(3087, 3485, 0);
				break;
			...
			case 565:
				...
				player.getPA().movePlayer(3084, 3500, 0);
				break;
			...
			case 567:
				...
				player.getPA().movePlayer(2272, 4059, 0);
				break;

			case 569:
				sendNpcChat("Check out the settings tab for many different client options.", "Customise your client to how you like it!", "Our Client goes Full Screen, we reccomend using Resizeable.", FacialAnimation.DEFAULT.getAnimationId());
				player.nextDialogue = 570;
				player.getPA().movePlayer(3087, 3493, 0);
				break;
			case 570:
				sendNpcChat("If you get tired of your look come to any", "Clothing shop, Make-Over Mage, or Hairdresser.", FacialAnimation.DEFAULT.getAnimationId());
				player.nextDialogue = 571;
				player.getPA().movePlayer(3084, 3511, 0);
				break;
			case 571:
				sendNpcChat("Access any bank across the world! In some areas you", "may also find Bank Chests which can be used as well.", "Deposit boxes also work & make for quick banking.", FacialAnimation.DEFAULT.getAnimationId());
				player.nextDialogue = 572;
				player.getPA().movePlayer(3097, 3493, 0);
				break;
			case 572:
				sendNpcChat("We have a massive map for all to explore!", "Be sure to look around & see what each city has to offer.", "Keep an eye out for quests around the world!", FacialAnimation.DEFAULT.getAnimationId());
				player.nextDialogue = 573;
				player.getPA().movePlayer(3222, 3218, 0);
				break;
			case 573:
				sendNpcChat("Click to continue!", FacialAnimation.DEFAULT.getAnimationId());
				player.nextDialogue = 574;
				player.getPA().movePlayer(2965, 3378, 0);
				break;
			case 574:
				sendNpcChat("Click to continue!", FacialAnimation.DEFAULT.getAnimationId());
				player.nextDialogue = 575;
				player.getPA().movePlayer(3269, 3167, 0);
				break;
			case 575:
				sendNpcChat("Yea.. That's right keep on clicking!", FacialAnimation.DEFAULT.getAnimationId());
				player.nextDialogue = 576;
				player.getPA().movePlayer(3494,3483, 0);
				break;	
			case 576:
				sendNpcChat("Whew! that was annoying... You get the point right?", "There is a lot to see & do! Anyhow let's keep going.", FacialAnimation.DEFAULT.getAnimationId());
				player.nextDialogue = 577;
				player.getPA().movePlayer(2661, 3303, 0);
				break;	
			case 577:
				sendNpcChat("Be sure to Sign up on our Forums! Runecessor.com", "Also keep an eye out for our events & quests!", "Oh! & don't forget to get on the Runecessor Discord!", FacialAnimation.DEFAULT.getAnimationId());
				player.nextDialogue = 578;
				player.getPA().movePlayer(3095, 3503, 0);
				break;
			case 578:
				sendNpcChat("Enjoy your stay " + player.getPlayerName() + ".", "Invite your friends over for even more fun!", "Invite friends for some special rewards too!", FacialAnimation.DEFAULT.getAnimationId());
				player.nextDialogue = 579;
				break;
			case 579:
				NewPlayerContent.endTutorial(player);
				break;
