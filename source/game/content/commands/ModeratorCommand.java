...
public class ModeratorCommand {
	public static boolean moderatorCommand(Player player, String command) {
		if (player.isStaffManagereratorRank()) {
		...
		if (command.startsWith("wealth")) {
			...
			try {
				...
				player.getPA().sendMessage("Use as ::wealth Lava (it only works on banned players)");
			}
			return true;
		...
		if (command.startsWith("ungambleban")) {
			...
			/*
			 * } else if (command.startsWith("07donatedamount")) { try { String parse[] =
			 * command.split(" "); double amount = Double.parseDouble(parse[1]); if (amount
			 * > WebsiteLogInDetails.OSRS_DONATION_LIMIT || amount <= 0) { return true; } if
			 * (!OsrsPaymentLimit.canAcceptOsrsPayment(player, (int)
			 * Misc.getDoubleRoundedUp(amount))) { return true; } String name =
			 * command.replace("07donatedamount", ""); name = name.replace(" " + parse[1] +
			 * " ", ""); double usdAmount = amount; usdAmount *=
			 * DonationManager.OSRS_DONATION_MULTIPLIER; double profit = (usdAmount *
			 * (DonationManager.PERCENTAGE_PROFIT_PER_MIL / 100.0)); profit =
			 * Misc.roundDoubleToNearestTwoDecimalPlaces(profit);
			 * DonationManager.totalPaymentsToday += profit; int tokens =
			 * DonationManager.getTokensAmountForUsd(usdAmount); boolean online = false; for
			 * (int i = 0; i < ServerConstants.MAXIMUM_PLAYERS; i++) { Player loop =
			 * PlayerHandler.players[i]; if (loop == null) { continue; } if
			 * (loop.getPlayerName().equalsIgnoreCase(name)) { online = true; if
			 * (ItemAssistant.addItem(loop, 7478, tokens)) {
			 * OsrsPaymentLimit.osrsPaymentLimitList.add(new OsrsPaymentLimit((int)
			 * Misc.getDoubleRoundedUp(amount), System.currentTimeMillis()));
			 ...
			 * Misc.getDateAndTime()), new StringParameter(2, loop.getPlayerName()), new
			 * DoubleParameter(3, profit), new StringParameter(4, "OSRS"));
			 * loop.getPA().sendMessage( ServerConstants.BLUE_COL + "You receive x" +
			 * Misc.formatNumber(tokens) + " Donator tokens for donating x" + amount +
			 * "m 07 which is " + Misc.roundDoubleToNearestTwoDecimalPlaces(usdAmount) +
			 * "$"); loop.getPA().sendMessage(ServerConstants.BLUE_COL +
			 * "Thank you so much for donating which will keep " +
			 * ServerConstants.getServerName() + " growing!");
			 * loop.getPA().sendMessage(ServerConstants.BLUE_COL +
			 * "Spend these at the Donator shop to earn a Donator rank or Sell the tokens");
			 * loop.getPA().sendMessage( ServerConstants.BLUE_COL + "to other players for "
			 * + TradingPost.getAverageTokenPrice() + " " +
			 * ServerConstants.getMainCurrencyName() .toLowerCase() +
			 * " each at the Donator npc trading post."); player.getPA().sendMessage(tokens
			 * + " Donator tokens awarded to: " + loop.getPlayerName() + " for donating: " +
			 * amount + "m."); int reward = -1; double rewardAmount = 1; int extraMills = 0;
			 * if (!DonationManager.botCheckRunning) { DonationManager.botCheckRunning =
			 * true; CycleEventHandler.getSingleton().addEvent(player, new CycleEvent() {
			 * 
			 ...
			 * BOT_FILE_LOCATION, "rsn=blank#" + System.currentTimeMillis());
			 * DonationManager.botCheckRunning = false; } }, ServerConfiguration.DEBUG_MODE
			 * ? 10 : 50); }
			 * 
			 * if (amount >= 80) { reward =
			 * MysteryBoxData.LEGENDARY_MYSTERY_BOX.getItemId(); rewardAmount = (double)
			 * amount / 80.0; rewardAmount = Misc.getDoubleRoundedDown(rewardAmount);
			 * extraMills = (int) Misc.getDoubleRoundedUp(amount) - (80 * (int)
			 * rewardAmount); } else if (amount >= 40) { reward =
			 * MysteryBoxData.SUPER_MYSTERY_BOX.getItemId(); extraMills = (int)
			 * Misc.getDoubleRoundedUp(amount) - 40; } else if (amount >= 20) { reward =
			 * MysteryBoxData.MYSTERY_BOX.getItemId(); extraMills = (int)
			 * Misc.getDoubleRoundedUp(amount) - 20; } else { extraMills = (int)
			 * Misc.getDoubleRoundedUp(amount); } if (reward > 0) {
			 * player.getPA().sendMessage(ServerConstants.GREEN_COL_PLAIN +
			 * "You are rewarded x" + (int) rewardAmount + " " +
			 * ItemAssistant.getItemName(reward) + " for being a trustworthy Moderator!");
			 * ItemAssistant.addItemReward(null, player.getPlayerName(), reward, (int)
			 * rewardAmount, false, -1); } if (extraMills > 0) { rewardAmount = extraMills *
			 * 1000; player.getPA().sendMessage(ServerConstants.GREEN_COL_PLAIN +
			 * "You are rewarded x" + (int) rewardAmount + " " +
			 * ItemAssistant.getItemName(13307) + "."); ItemAssistant.addItemReward(null,
			 * player.getPlayerName(), 13307, (int) rewardAmount, false, -1); }
			 * DonationManager.osrsReceivedThisServerSession += amount;
			 * DonationManager.setOsrsInInventory(Misc.roundDoubleToNearestOneDecimalPlace(
			 * DonationManager.getOsrsInInventory() + Misc.getDoubleRoundedUp(amount)));
			 * DonationManager.osrsToday += amount; SQLNetwork.insert("INSERT INTO " +
			 * SQLConstants.getServerSchemaTable(SQLConstants.STATS_OSRS_HISTORY) +
			 * " (time, entity, action, total_expected) VALUES(?, ?, ?, ?)", new
			 * StringParameter(1, Misc.getDateAndTime()), new StringParameter(2, "Mod"), new
			 * StringParameter(3, player.getPlayerName() + " has taken " + amount +
			 * "m osrs from " + loop.getPlayerName()), new DoubleParameter(4,
			 ...
			 * 
			 * // Email ArrayList<String> content = new ArrayList<String>();
			 * content.add("Moderator: " + player.getPlayerName());
			 * content.add("Moderator ip: " + player.addressIp);
			 * content.add("Moderator uid: " + player.addressUid); content.add("Donator: " +
			 * loop.getPlayerName()); content.add("Donator ip: " + loop.addressIp);
			 * content.add("Donator uid: " + loop.addressUid); content.add("Donated: " +
			 * amount + "m for " + tokens + " Donator tokens");
			 * EmailSystem.addPendingEmail("Osrs: " + DonationManager.getOsrsInInventory() +
			 * "m, payment by Mod: " + amount + "m", content, "wecare2018s");
			 * 
			 * } else { player.getPA().sendMessage("His inventory is too full.");
			 * loop.getPA().sendMessage("Get an inventory space to receive your donation!");
			 * } break; } } if (!online) { player.getPA().sendMessage(name +
			 * " is not online."); }
			 * 
			 * return true; } catch (Exception e) { player.getPA().
			 * sendMessage("Use as ::07donatedamount 50 Lava, which means the player donated 50m 07 gold."
			 * ); return true; }
			 */
		}
		switch (command) {
