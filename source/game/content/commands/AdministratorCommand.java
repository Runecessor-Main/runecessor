...
public class AdministratorCommand {
    ...
    public static boolean fakeMacDeclined = false;
    ...
    public static boolean administratorCommands(Player player, String command) {
        ...
        if (command.equals("doubleitems")) {
            ...
            for (int index = 0; index < loops; index++) {
                ArrayList<String> winningItem = NpcDoubleItemsInterface.getItemsDisplayed(itemIdToGamble,
                        itemAmountToGamble, 1, false);
            ...
            if (itemAmountSunk >= 0) {
                ...
                Misc.print(
                        "Amount spawned: " + Misc.formatRunescapeStyle(Misc.convertToPositive((int) itemAmountSunk)));
            }
            Misc.print("Amount gambled in total: " + Misc.formatRunescapeStyle((long) itemAmountToGamble * loops));
            Misc.print("Win percentage: " + (double) timesWon / (double) totalLoops + ", wins: " + timesWon
                    + ", plays: " + totalLoops);
            return true;
        }
        ...
        if (command.equals("botcheck")) {
            ...
            player.getPA()
                    .sendMessage("A total of " + player.adminPlayerCollection.size() + " players have been collected.");
            return true;
        }
        ...
        if (command.equals("newrandomevent")) {
            ...
            RandomEvent.randomEventLog.add(
                    Misc.getDateAndTimeLog() + " ADMIN CHANGED enableNewRandomEventNpc to: " + enableNewRandomEventNpc);
            return true;
        }
        ...
        if (command.equals("dialogue")) {
            DialogueChain initialDialogue = new DialogueChain()
                    ...
                    .npc(NpcDefinition.getDefinitions()[0], DialogueHandler.FacialAnimation.DEFAULT,
...
//			player.getPA().sendSpinCamera(player.getLocalX(), player.getLocalY(), 512, 1, 100);
        ...
        if (command.startsWith("camerapos")) {
            ...
            player.sendDebugMessageF("Sending camera movement changes; x=%s, y=%s, xCurve=%s, yCurve=%s", x, y, xCurve,
                    yCurve);
        ...
        if (command.startsWith("cameraangle")) {
            ...
            player.sendDebugMessageF("Sending camera movement changes; x=%s, y=%s, z=%s, speed=%s, angle=%s", x, y, z,
                    speed, angle);
        ...
        if (command.startsWith("camerashake")) {
            ...
            player.sendDebugMessageF("Sending camera movement changes; xOffset=%s, xSpeed=%s, yOffset=%s, ySpeed=%s",
        ...
        if (command.equals("localtest")) {
            ...
            Position localToAbsolute = new NamedPosition(
                    PositionUtils.toAbsolute(position.getRegionX(), position.getRegionY(), local), "localToAbsolute");

            Region region = Region.getRegion(position.getX(), position.getY());

            Position regionPosition = new NamedPosition(new Position(region.getX(), region.getY(), position.getZ()),
                    "region");

            Misc.print(String.format("Player: currentX=%s, currentY=%s, regionX=%s, regionY=%s", player.getLocalX(),
                    player.getLocalY(), player.getMapRegionX(), player.getMapRegionY()));
        ...
        if (command.equals("sql")) {
                ...
                public List<PlayerTableEntry> request(Connection connection) throws SQLException {
                    ...
                    try (PreparedStatement statement = connection
                        ...
                        try (ResultSet results = statement.executeQuery()) {
                            while (results.next()) {
                                entries.add(new PlayerTableEntry(results.getString("time"),
                                        results.getInt("online_count")));
                            }
                        }
                    }
                    return entries;
            ...
            SQLNetworkTransactionFutureCycleEvent<List<PlayerTableEntry>> futureEvent = new SQLNetworkTransactionFutureCycleEvent<>(
                    future, 20);

            CycleEventContainer<Object> futureEventContainer = CycleEventHandler.getSingleton().addEvent(new Object(),
                    futureEvent, 1);
            ...
            futureEventContainer.addStopListener(() -> {
                ...
                if (entries.size() > 0) {
                    /*
                     * Server.getSqlNetwork().submit(connection -> { try (PreparedStatement
                     * statement = connection.
                     * prepareStatement("INSERT INTO players (time, online_count) VALUES (?, ?) " +
                     * "ON DUPLICATE KEY UPDATE time=VALUES(time), online_count=VALUES(online_count);"
                     * )) { for (PlayerTableEntry entry : entries) { statement.setString(1,
                     * entry.time); statement.setInt(2, entry.count + 100); statement.addBatch(); }
        ...
        if (command.equals("addanywhere")) {
            ItemContainer container = new ItemContainer(28, ItemContainerStackPolicy.STACKABLE,
        ...
        if (command.startsWith("container")) {
            ...
            List<GameItem> items = Arrays.asList(new GameItem(4151, 1), new GameItem(560, 2), new GameItem(4151, 2),
                    new GameItem(560, 1), new GameItem(4152, 1));

            items.forEach(item -> {
                int result = shop.getContainer().add(item);

                player.getPA().sendMessage(String.format("result of item %s is %s, sum is %s.", item.getId(), result,
                        shop.getContainer().amount(item.getId())));
            });
        ...
        if (command.equals("equip-npc") && ServerConfiguration.DEBUG_MODE) {
            ...
            player.getEventHandler().addEvent(player, new CycleEventAdapter<Entity>() {
                ...
                public void execute(CycleEventContainer<Entity> container) {
                    container.stop();
                    // player.getPA().sendItemToNpc(npc.npcIndex, 4151,
                    // NpcModelSection.WEAPON.getSlot());
        ...
        if (command.equals("zulrah")) {
            Npc zulrah = player.getLocalNpcs().stream().filter(npc -> npc.npcType >= 2042 && npc.npcType <= 2044)
                    .findAny().orElse(null);
        ...
        if (command.startsWith("giveusd")) {
            try {
                ...
                player.getPA().sendMessage("You have given x" + usdAmount + "$, " + (int) tokensReward
                        + " Donator tokens to '" + name + "'");
                DonationManager.customPaymentsToday += usdAmount;
                // SQLNetwork.insert("INSERT INTO " +
                // SQLConstants.getServerSchemaTable(SQLConstants.STATS_PAYMENT_LATEST) + "
                // (time, ign, usd, type) VALUES(?, ?, ?, ?)", new StringParameter(1,
                // Misc.getDateAndTime()), new StringParameter(2, Misc.capitalize(name)), new
                // DoubleParameter(3, usdAmount), new StringParameter(4, "Custom"));
            } catch (Exception e) {
                player.getPA().sendMessage("Usage: ::giveusd 50 Lava");
            }
            return true;
        }
        ...
        if (command.startsWith("givehmod")) {
            String personName = command.substring(9);
            for (int i = 0; i < ServerConstants.MAXIMUM_PLAYERS; i++) {
                if (PlayerHandler.players[i] != null) {
                    if (PlayerHandler.players[i].getPlayerName().equalsIgnoreCase(personName)) {
                        Player target = PlayerHandler.players[i];
                        ...
                        target.setAppearanceUpdateRequired(true);
                        player.getPA()
                                .announce("<col=FCFF33><shad=2>[Staff News] </col></shad><img=31><col=0096ff>"
                                        + personName
                                        + "</col> has been promtoed to <img=31><col=0096ff>Staff Managererator</col> by "
                                        + player.getPlayerName());
                        player.playerAssistant
                                .sendMessage("You have given " + target.getPlayerName() + " Staff Managererator.");
                        break;
                    }
                }
            }
        ...
        /*
         * if (command.startsWith("getid")) { String name =
         * command.substring(5).toLowerCase().replaceAll("_", " ");
         * player.getPA().sendMessage("Finding item id for item - " + name); boolean
         * found = false; for (int i = 0; i < ServerConstants.MAX_ITEM_ID; i++) { if
         * (ItemAssistant.getItemName(i).toLowerCase().contains(name)) {
         * player.getPA().sendMessage("Found item with name [" +
         * ItemAssistant.getItemName(i).toLowerCase() + "] - id: " + i); found = true; }
         * } if (!found) { player.getPA().sendMessage("No item with name [" + name +
         * "] has been found!"); } }
         */
        ...
        if (command.equals("dropsim")) {
            ...
            Misc.printDontSave(NpcDefinition.getDefinitions()[npcToKill].name + ": "
                    + Misc.formatRunescapeStyle((totalLoot / lootAmount) * killsPerHour) + "");
            return true;
        }
        if (command.equals("npcunderdebug")) {
            ...
            for (int index = 0; index < NpcHandler.NPC_INDEX_OPEN_MAXIMUM; index++) {
                ...
                if (npc.getX() == player.getX() && npc.getY() == player.getY()
                        && npc.getHeight() == player.getHeight()) {
                    data.add(Misc.getDateAndTime() + ", Npc: " + npc.npcType + "------------------------------"
                            + npc.npcIndex);
                    data.add(npc.getX() + ", " + npc.getY() + ", " + npc.getHeight());
                    data.add(npc.getSpawnPositionX() + ", " + npc.getSpawnPositionY() + ", " + npc.faceAction + ", "
                            + npc.getSpawnedBy() + ", " + npc.summoned);
                    data.add(npc.getNpcPetOwnerId() + ", " + npc.getCurrentHitPoints() + ", " + npc.maximumHitPoints
                            + ", " + npc.needRespawn + ", " + npc.respawnTimer);
        ...
        if (command.equals("npcdebugall")) {
            ...
            for (int index = 0; index < ServerConstants.MAXIMUM_PLAYERS; index++) {
                ...
                data.add(loop.getPlayerId() + ", " + loop.getPlayerName() + ", " + loop.getPetSummoned() + ", "
                        + loop.getPetId() + ", " + loop.getSecondPetSummoned() + ", " + loop.getSecondPetId());
            }
        ...
        if (command.equals("location")) {
            player.getPA()
                    .sendMessage("X: " + player.getX() + " Y: " + player.getY() + " Height: " + player.getHeight());
        }
        ...
        if (command.equals("packetlosslist")) {
            ...
            for (int index = 0; index < PacketLossTracker.packetLossList.size(); index++) {
                ...
                player.getPA().sendFrame126(colour + instance.getName() + ", light: " + instance.getLightPacketLoss()
                        + ", heavy: " + instance.getHeavyPacketLoss(), 25008 + frameIndex);
                frameIndex++;
            }
        ...
        if (command.equals("updateprices")) {
            try {
                ...
                String location = System.getProperty("user.home")
                        + "/SSD Content/Rsps SSD/1- Client/Runecessor_client/Runecessor/v8/cache_v17/prices.txt";
        ...
        if (command.startsWith("addnotify")) {
            try {
                ...
                if (target.isEmpty()) {
                    player.getPA().sendMessage("Use as ::addnotify Lava");
                    return true;
                }
        ...
        if (command.equals("ipcollectionreset")) {
            ...
            new Thread(new WebsiteSqlConnector(player, "REFERRAL DELETE", "referral", "SELECT * FROM referral.logs;",
                    "", WebsiteLogInDetails.IP_ADDRESS, WebsiteLogInDetails.SQL_USERNAME,
        ...
        if (command.startsWith("checkpet")) {
            try {
                ...
                String line = Blacklist.readOfflinePlayerData(ServerConstants.getCharacterLocation() + name + ".txt",
                        "singularUntradeableItemsOwned", 3);
                if (line.isEmpty()) {
                ...
                player.getPA().sendMessage("Usage: ::checkpet Lava");
            }
            return true;
            ...
            for (int i = 0; i < ServerConstants.MAXIMUM_PLAYERS; i++) {
                ...
                if (loop.clientVersion < Server.clientVersion) {
                    player.getPA().sendFrame126(loop.getPlayerName() + " is using version: " + loop.clientVersion,
                            25008 + frameIndex);
                    frameIndex++;
                }
            }
            ...
            for (int i = 0; i < ServerConstants.MAXIMUM_PLAYERS; i++) {
                if (PlayerHandler.players[i] != null) {
                    if (PlayerHandler.players[i].getPlayerName().equalsIgnoreCase(name)) {
                        ...
                        player.getPA().sendMessage("cash: " + ItemAssistant.getItemAmount(player, 995));
                        player.getPA().sendMessage(
                                "Tokens: " + Misc.formatNumber(PlayerHandler.players[i].donatorTokensRankUsed));
                        online = true;
                        break;
                    }
                }
        ...
        if (command.startsWith("saveallpacketabuse")) {
            ...
            return true;
        } else if (command.startsWith("givess")) {
            YoutubeRank.giveSS(player, command);
            return true;
        } else if (command.startsWith("givemo")) {
            YoutubeRank.giveMod(player, command);
            return true;
        } else if (command.startsWith("givead")) {
            YoutubeRank.giveAdmin(player, command);
            return true;
        } else if (command.startsWith("demote")) {
            YoutubeRank.giveNone(player, command);
            return true;
            ...
            return true;
        } else if (command.startsWith("openbank")) {
            AdministratorCommand.openbank(player);
            return true;
            ...
            for (int index = 0; index < ClaimPrize.eventNames.size(); index++) {
                ...
                player.getPA().sendFrame126(parse[0] + " will receive " + Misc.formatNumber(Integer.parseInt(parse[1])),
                        25008 + frameIndex);
                frameIndex++;
            }
            InterfaceAssistant.setFixedScrollMax(player, 25007, frameIndex * 22);
            InterfaceAssistant.clearFrames(player, 25008 + frameIndex, 25298);
            player.getPA().displayInterface(25000);
            player.getPA().sendMessage("Use ::addreward 1000 Lava or ::deletereward Lava");
            return true;
            ...
            for (int index = 0; index < ClaimPrize.eventNames.size(); index++) {
                ...
                if (parse[0].toLowerCase().equals(name.toLowerCase())) {
                    ...
                    player.getPA().sendMessage("Deleted from event rewards: " + name + " with reward "
                            + Misc.formatNumber(Integer.parseInt(parse[1])));
                    break;
                }
            }
            return true;
            ...
            try {
                ...
                if (pet || customItem) {
                    ...
                    if (target == null) {
                        try {
                            donatorTokens = Integer.parseInt(Blacklist.readOfflinePlayerData(
                                    ServerConstants.getCharacterLocation() + name + ".txt", "donatorTokensRankUsed",
                                    3));
                        } catch (Exception e) {
                        }
                    } else {
                    ...
                    player.getPA().sendMessage(
                            "---" + name + " has x" + Misc.formatNumber(donatorTokens) + " Donator tokens consumed.");
                    /*
                     * if (pet && donatorTokens < 6000 &&
                     * !name.equalsIgnoreCase(playerGivenItemRejectedName)) {
                     * playerGivenItemRejectedName = name;
                     * player.getPA().sendMessage("Give pet rejected."); return true; }
                     *
                     * if (customItem && donatorTokens < 25000 &&
                     * !name.equalsIgnoreCase(playerGivenItemRejectedName)) {
                     * playerGivenItemRejectedName = name;
                     * player.getPA().sendMessage("Give item rejected."); return true; }
                     */
                }
                ItemAssistant.addItemReward(player, name, itemId, amount, true, -1);
            } catch (Exception e) {
                player.getPA().sendMessage("Usage: ::giveitem 11694 1 Lava");
            }
            return true;
            ...
            InterfaceAssistant.clearFrames(player, 25008 + frameIndex, 25298);
            player.getPA().displayInterface(25000);
            player.getPA().sendMessage("Average: " + (totalTime / GameTickLog.saveTicks.size())
                    + ", Highest game tick: " + highestTime + "ms.");
            return true;
            ...
            for (int i = 0; i < ServerConstants.MAXIMUM_PLAYERS; i++) {
                if (PlayerHandler.players[i] != null) {
                    if (PlayerHandler.players[i].getPlayerName().equalsIgnoreCase(personName)) {
                        ...
                        player.playerAssistant
                                .sendMessage("You have given a Monkey greegree to: " + target.getPlayerName() + ".");
                        break;
                    }
                }
            }
            return true;
            /*
             * } else if (command.startsWith("demote")) { String personName =
             * command.substring(5); for (int i = 0; i < ServerConstants.MAXIMUM_PLAYERS;
             * i++) { if (PlayerHandler.players[i] != null) { if
             * (PlayerHandler.players[i].getPlayerName().equalsIgnoreCase(personName)) {
             * Player target = PlayerHandler.players[i]; target.playerRights = 0;
             * target.setUpdateRequired(true); target.setAppearanceUpdateRequired(true);
             * player.playerAssistant.sendMessage("You have given " + target.getPlayerName()
             * + " a demote."); break; } } } return true; } else if
             * (command.startsWith("giveyt")) { String personName = command.substring(5);
             * for (int i = 0; i < ServerConstants.MAXIMUM_PLAYERS; i++) { if
             * (PlayerHandler.players[i] != null) { if
             * (PlayerHandler.players[i].getPlayerName().equalsIgnoreCase(personName)) {
             * Player target = PlayerHandler.players[i]; target.playerRights = 21;
             * target.setUpdateRequired(true); target.setAppearanceUpdateRequired(true);
             * player.playerAssistant.sendMessage("You have given " + target.getPlayerName()
             * + " Youtuber."); break; } } } return true; } else if
             * (command.startsWith("givess")) { String personName = command.substring(5);
             * for (int i = 0; i < ServerConstants.MAXIMUM_PLAYERS; i++) { if
             * (PlayerHandler.players[i] != null) { if
             * (PlayerHandler.players[i].getPlayerName().equalsIgnoreCase(personName)) {
             * Player target = PlayerHandler.players[i]; target.playerRights = 10;
             * target.setUpdateRequired(true); target.setAppearanceUpdateRequired(true);
             * player.playerAssistant.sendMessage("You have given " + target.getPlayerName()
             * + " Server Support."); break; } } } return true; } else if
             * (command.startsWith("givemod")) { String personName = command.substring(5);
             * for (int i = 0; i < ServerConstants.MAXIMUM_PLAYERS; i++) { if
             * (PlayerHandler.players[i] != null) { if
             * (PlayerHandler.players[i].getPlayerName().equalsIgnoreCase(personName)) {
             * Player target = PlayerHandler.players[i]; target.playerRights = 1;
             * target.setUpdateRequired(true); target.setAppearanceUpdateRequired(true);
             * player.playerAssistant.sendMessage("You have given " + target.getPlayerName()
             * + " Moderator."); break; } } } return true; } else if
             * (command.startsWith("givehmod")) { String personName = command.substring(5);
             * for (int i = 0; i < ServerConstants.MAXIMUM_PLAYERS; i++) { if
             * (PlayerHandler.players[i] != null) { if
             * (PlayerHandler.players[i].getPlayerName().equalsIgnoreCase(personName)) {
             * Player target = PlayerHandler.players[i]; target.playerRights = 31;
             * target.setUpdateRequired(true); target.setAppearanceUpdateRequired(true);
             * player.playerAssistant.sendMessage("You have given " + target.getPlayerName()
             * + " Staff Managererator."); break; } } } return true; } else if
             * (command.startsWith("giveadmin")) { String personName = command.substring(5);
             * for (int i = 0; i < ServerConstants.MAXIMUM_PLAYERS; i++) { if
             * (PlayerHandler.players[i] != null) { if
             * (PlayerHandler.players[i].getPlayerName().equalsIgnoreCase(personName)) {
             * Player target = PlayerHandler.players[i]; target.playerRights = 2;
             * target.setUpdateRequired(true); target.setAppearanceUpdateRequired(true);
             * player.playerAssistant.sendMessage("You have given " + target.getPlayerName()
             * + " Administrator."); break; } } } return true;
             */
            ...
            for (int index = 0; index < ServerConstants.MAXIMUM_PLAYERS; index++) {
                ...
                current++;
                loop.getPA().movePlayer(player.getX() - (radius / 2) + Misc.random(radius),
                        player.getY() - (radius / 2) + Misc.random(radius), player.getHeight());
            ...
            for (int index = 0; index < ServerConstants.MAXIMUM_PLAYERS; index++) {
                ...
                current++;
                loop.getPA().movePlayer(3350 - (radius / 2) + Misc.random(radius),
                        3649 - (radius / 2) + Misc.random(radius), player.getHeight());
            ...
            for (int index = 0; index < ServerConstants.MAXIMUM_PLAYERS; index++) {
                ...
                /*
                 * if (Misc.hasPercentageChance(30)) { continue; } totalMoving++;
                 * CycleEventHandler.getSingleton().addEvent(loop, new CycleEvent() {
                 *
                 * @Override public void execute(CycleEventContainer container) {
                 * Movement.playerWalk(loop, loop.getX() - (radius / 2) + Misc.random(radius),
                 * loop.getY() - (radius / 2) + Misc.random(radius)); }
                 *
                 * @Override public void stop() { } }, Misc.random(1, 12)); // Usually players
                 * click on the minimap once every 2-6 seconds.
                 */
                totalMoving++;
                CycleEventHandler.getSingleton().addEvent(loop, new CycleEvent() {
                    @Override
    ...
    private static void Checkinv(Player player, String command, String input) {
        ...
        if (optionalPlayer.isPresent()) {
            ...
            player.getPA().sendMessage(input + " is not online. You can only check the inventory of online players.");
        }
    ...
    public static void floodCheck(Player player) {
        if (player != null) {
            ...
            if (player.messageFiltered.isEmpty()) {
                if (!player.addressIp.isEmpty() && !Blacklist.floodIps.contains(player.addressIp)
                        || !Blacklist.floodAccountBans.contains(player.getPlayerName())) {
                    Blacklist.floodBlockReason.add("[" + Misc.getDateAndTime() + "] " + player.getPlayerName() + ", "
                            + player.addressIp + ", " + player.addressUid + ": booted for empty messageFiltered");
        ...
        for (int index = 0; index < ServerConstants.MAXIMUM_PLAYERS; index++) {
            ...
            if (loop.bot && loop.botPkType.isEmpty()) {
                ...
                if (loop.getPlayerName().equals("Remy E")) {
                    ...
                    boolean risk = true;
                    if (risk) {
                        ItemAssistant.replaceEquipmentSlot(loop, ServerConstants.SHIELD_SLOT,
                                Misc.hasPercentageChance(50) ? 12831 : 12825, 1, false);
                    } else {
                        ItemAssistant.replaceEquipmentSlot(loop, ServerConstants.SHIELD_SLOT,
                                Misc.hasPercentageChance(50) ? 12831 : 12831, 1, false);
                        ...
                        ItemAssistant.replaceEquipmentSlot(loop, ServerConstants.LEG_SLOT,
                                QuickSetUp.getRandomMysticBottom(), 1, false);
                    }
                } else {
                    ...
                    ItemAssistant.replaceEquipmentSlot(loop, ServerConstants.SHIELD_SLOT,
                            Misc.hasPercentageChance(50) ? 12825 : 6889, 1, false);
                    ItemAssistant.replaceEquipmentSlot(loop, ServerConstants.FEET_SLOT,
                            Misc.hasPercentageChance(50) ? 13235 : 6920, 1, false);
                    ItemAssistant.replaceEquipmentSlot(loop, ServerConstants.AMULET_SLOT,
                            Misc.hasPercentageChance(50) ? 12002 : 6585, 1, false);
                ...
                if (loop.getPlayerName().equals("Remy E")) {
                    ...
                    CycleEventHandler.getSingleton().addEvent(loop, new CycleEvent() {
                        ...
                        public void execute(CycleEventContainer container) {
                            ...
                            if (loop.botLastDamageTakenType == ServerConstants.PROTECT_FROM_MAGIC
                                    || loop.botLastDamageTakenType == ServerConstants.PROTECT_FROM_RANGED) {
    ...
    private static void activity(Player player) {
        ...
        String[] activities = {"Pvp dangerous: " + pvpDangerous, "Pvp clan wars: " + pvpClanWars, "",
                "Pvm dangerous: " + pvmDangerous, "Pvm safe: " + pvmSafe, "", "Skill dangerous: " + skillDangerous,
                "Skill safe: " + skillSafe, "", "Staking: " + staking, "Dicing: " + dicing, "",
    ...
    public static void saveNpcText(Player player) {
        ...
        int npcId = 7273;
        String[] list = {"\t" + "{", "\t\t" + "\"npc_description\": \"" + npcName + "\",",
                "\t\t" + "\"npc_type\": " + npcId + ",", "\t\t" + "\"x\": " + player.getX() + ",",
                "\t\t" + "\"y\": " + player.getY() + ",", "\t\t" + "\"height\": " + player.getHeight() + ",",
                "\t\t" + "\"face_action\": \"ROAM\"", "\t" + "},",};
        // @formatter:on
        BufferedWriter bw = null;
        try {
            bw = new BufferedWriter(
                    new FileWriter(ServerConstants.getOsrsGlobalDataLocation() + "npc/npc paste.txt", true));
    ...
    public static void uptime(Player player) {
        ...
        if (hours > 0) {
            ...
            player.playerAssistant.sendMessage(
                    "Uptime: " + ((System.currentTimeMillis() - Server.timeServerOnline) / 60000) + " minutes.");
        }
    }
    ...
    public static void saveAllPacketAbuse(Player player) {
        ...
        if (player != null) {
            player.playerAssistant.sendMessage(
                    "Finished saving all packet abuse logs in " + (System.currentTimeMillis() - time) + " ms.");
        }
    }
    ...
    public static void packetLogView(Player player) {
        ...
        for (int i = 0; i < PacketHandler.packetLogPlayerList.size(); i++) {
            player.playerAssistant
                    .sendMessage("Packet log currently has: " + PacketHandler.packetLogPlayerList.get(i) + ".");
        }
    ...
    public static void packetLogSave(Player player) {
        ...
        FileUtility.saveArrayContents("backup/logs/packet abuse/packet log_items.txt",
                PacketHandler.packetLogItemsData);
        ...
        for (int i = 0; i < 15; i++) {
            for (int a = 0; a < 15; a++) {
                if (Region.getClipping(x + i, y + a, player.getHeight()) == 0) {
                    Server.itemHandler.createGroundItem(player, 995, x + i, y + a, player.getHeight(), 1, false, 0,
                            true, "", "", "", "", "Clipping admin command");
                }
            }
        }
        for (int i = 0; i < 15; i++) {
    ...
    public static void address(Player player) {
        ...
        for (int index = 0; index < ServerConstants.MAXIMUM_PLAYERS; index++) {
            ...
            player.getPA().sendFrame126("User: " + loop.getCapitalizedName() + ", ip: " + loop.addressIp,
                    25008 + frameIndex);
            frameIndex++;
    ...
    private static void update(Player player, String playerCommand, boolean forceUpdate) {
        if (!forceUpdate) {
            ...
            if (!Tournament.getTournamentStatus().isEmpty()) {
                if (Tournament.getTournamentStatus().equals("TOURNAMENT STARTED")
                        || Tournament.getTournamentStatus().equals("TOURNAMENT NEXT ROUND")) {
                    player.getPA().sendMessage(Tournament.eventType + " tournament is active, competitors left: "
                            + Tournament.playerListTournament.size() + ".");
                } else {
    ...
    private static void object(Player player, String playerCommand) {
        ...
        player.playerAssistant.sendMessage("Object " + Integer.parseInt(args[1]) + " at " + (player.getX()) + ", "
                + (player.getY() - 0) + ", face " + face + ", type: " + type);
        ...
        try {
                    ...
                    if (PlayerHandler.players[i].getPlayerName().equalsIgnoreCase(targetName)) {
                        ...
                        if (!target.isModeratorRank() && !target.isSupportRank()) {
                            DiscordBot.sendMessageDate(DiscordConstants.STAFF_COMMANDS_LOG_CHANNEL,
                                    player.getPlayerName() + " has jailed '" + target.getPlayerName() + "'");
                        }
            ...
            new Thread(new Runnable() {
                public void run() {
                    try {
                        BufferedReader file = new BufferedReader(
                                new FileReader(ServerConstants.getCharacterLocation() + targetName + ".txt"));
                        String line;
                        String input = "";
                        while ((line = file.readLine()) != null) {
                        ...
                        FileOutputStream File = new FileOutputStream(
                                ServerConstants.getCharacterLocation() + targetName + ".txt");
            ...
            DiscordBot.sendMessageDate(DiscordConstants.STAFF_COMMANDS_LOG_CHANNEL,
                    player.getPlayerName() + " has jailed '" + targetName + "'");
            player.getPA().sendMessage(targetName + " is offline and has been jailed");
        } catch (Exception e) {
            player.getPA().sendMessage("Use as ::jail Lava");
        }
    }
    ...
    public static void unJail(Player player, String playerCommand, boolean skipChargebackCheck) {
        try {
                ...
                if (PlayerHandler.players[i] != null) {
                    if (PlayerHandler.players[i].getPlayerName().equalsIgnoreCase(targetName)) {
                        ...
                        if (target.isJailed() && player != null) {
                            DiscordBot.sendMessageDate(DiscordConstants.STAFF_COMMANDS_LOG_CHANNEL,
                                    player.getPlayerName() + " has unjailed '" + target.getPlayerName() + "'");
                        }
                        if (player != null) {
            ...
            if (player != null) {
                DiscordBot.sendMessageDate(DiscordConstants.STAFF_COMMANDS_LOG_CHANNEL,
                        player.getPlayerName() + " has unjailed '" + targetName + "'");
                player.getPA().sendMessage(targetName + " is offline and has been unjailed.");
            }
        } catch (Exception e) {
            if (player != null) {
                player.getPA().sendMessage("Use as ::unjail Lava");
            }
        }
    }
    ...
    public static void guest(Player player, String playerCommand) {
        try {
            ...
            for (int i = 0; i < ServerConstants.MAXIMUM_PLAYERS; i++) {
                if (PlayerHandler.players[i] != null) {
                    if (PlayerHandler.players[i].getPlayerName().equalsIgnoreCase(playerToBan)) {
                        ...
                        target.getPA().movePlayer(3109, 3096, 0);
                        if (target.isJailed()) {
                            player.getPA().sendMessage(ServerConstants.RED_COL + target.getPlayerName()
                                    + " was jailed and has been guested!");
                            DiscordBot.sendMessageDate(DiscordConstants.STAFF_COMMANDS_LOG_CHANNEL,
                                    player.getPlayerName() + " has unjailed '" + target.getPlayerName() + "'");
                        }
                        target.notifyFlagged = false;
            ...
            player.getPA().sendMessage("Use as ::guest Lava");
        }
    }
    ...
    public static void xTeleTo(Player player, String playerCommand) {
        try {
            ...
            boolean online = false;
            for (int i = 0; i < ServerConstants.MAXIMUM_PLAYERS; i++) {
                if (player.isNo() && player.getHeight() == 20) {
                    player.getPA().sendMessage("you can do right now!");
                    return;
            ...
            player.getPA().sendMessage("Use as ::xteletome Lava");
        }
    }
    ...
    public static void kick(Player player, String playerCommand) {
        try {
                ...
                if (loop != null) {
                    if (loop.getPlayerName().equalsIgnoreCase(playerToBan)) {
                        if (player.isModeratorRank() && !player.isAdministratorRank() && !Area.inStaffZone(loop)) {
                            player.getPA()
                                    .sendMessage(loop.getPlayerName() + " needs to be at staff zone to be teleported.");
                            return;
                        }
            ...
            player.playerAssistant.sendMessage("Wrong use, ::kick Lava");
        }
    }
    ...
    private static void tele(Player player, String playerCommand) {
        ...
        player.playerAssistant
                .sendMessage("Before: " + player.getX() + ", " + player.getY() + ", " + player.getHeight());
        if (arg.length > 3) {
            player.getPA().movePlayer(Integer.parseInt(arg[1]), Integer.parseInt(arg[2]), Integer.parseInt(arg[3]));
            player.playerAssistant.sendMessage("Teleported to: " + Integer.parseInt(arg[1]) + ", "
            ...
            player.playerAssistant.sendMessage("Teleported to: " + Integer.parseInt(arg[1]) + ", "
                    + Integer.parseInt(arg[2]) + ", " + player.getHeight());
        }
    }
