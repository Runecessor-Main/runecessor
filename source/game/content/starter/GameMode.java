 ...
 * Game modes.
 *
 * @author Lava, created on 05-07-2015.
 */
public class GameMode {
	...
	public static double getDropRate(Player player, double chance) {
		...
		if (player.isSupremeDonator()) {
			...
			modifier -= 0.03;
		} else if (player.getPetId() == 11108) {//
			modifier -= 0.15;
		} else if (player.getPetId() == 11042) {//
			modifier -= 0.25;
		} else if (player.playerEquipment[ServerConstants.RING_SLOT] == 20786) {
			modifier -= 0.12;
		} else if (player.playerEquipment[ServerConstants.AMULET_SLOT] == 589) {
			modifier -= 0.12;
		} else if (player.playerEquipment[ServerConstants.RING_SLOT] == 2572) {
			modifier -= 0.01;
		} else if (player.playerEquipment[ServerConstants.RING_SLOT] == 12785) {
			modifier -= 0.03;
		} else if (player.playerEquipment[ServerConstants.RING_SLOT] == 16505) {
			modifier -= 0.07;
	...
	/*
	 * public static double getDropRate(Player player, double chance) { double
	 * modifier = 1.0; double chanceNew = (double) chance; if (player.getPetId() ==
	 * 11108) { modifier -= 0.10; } else if (player.getPetId() == 11042) { modifier
	 * -= 0.25; } else if (GameMode.getDifficulty(player, "GLADIATOR")) { modifier
	 * -= 0.05; } else if (player.playerEquipment[ServerConstants.RING_SLOT] ==
	 * 2572) { modifier -= 0.01; } else if
	 * (player.playerEquipment[ServerConstants.RING_SLOT] == 12785) { modifier -=
	 * 0.03; } else if (player.playerEquipment[ServerConstants.RING_SLOT] == 20786)
	 * { modifier -= 0.15;// Added by Michael + 30% droprate } else if
	 * (player.playerEquipment[ServerConstants.RING_SLOT] == 16505) { modifier -=
	...
	public static void sendDescription(Player player, String starterType) {
		...
		if (starterType.equals("NORMAL")) {
			text.add("x200 combat experience");
			text.add("x75 skilling experience rate.");
			text.add("@gre@Ability to set combat levels after");
			text.add("@gre@maxed combat.");
			interfaceIdStart = 23476;
		}
		if (starterType.equals("GLADIATOR")) {
			text.add("x40 combat experience");
			text.add("x20 skilling experience rate.");
			//text.add("@gre@Ability to set combat levels after");
		//	text.add("@gre@maxed combat.");
		...
		//	text.add("+20% more coins from voting.");
			...
			text.add("@red@[@WARNING@] - GAME MODE DISABLED");
			...
			text.add("Cannot receive any item that belongs");
			text.add("to another player.");
			text.add("@red@You only have 1 life!");
			text.add("@or3@A dangerous death will downgrade your");
			text.add("@or3@game mode to Standard Ironman.");
			interfaceIdStart = 23512;
		}
	...
	public static boolean isGameModeButton(Player player, int buttonId) {
		...
		case 91160:
			...
			player.getPA().sendMessage("This game mode is disabled.");
			/*
			 * player.selectedGameMode = "PKER"; sendDescription(player, "PKER");
	...
	public static void announceMaxedCombatOrMaxedTotal(Player player) {
		if (player.getCombatLevel() == 126 && player.baseSkillLevel[ServerConstants.PRAYER] == 99
				&& player.loggingInFinished && !player.getAbleToEditCombat()) {
			// player.getPA().announce(GameMode.getGameModeName(player) + " has finally
			// achieved maxed combat.");
			// player.getPA().sendScreenshot("maxed combat", 2);
			// return;

		}

		if (Skilling.getOriginalTotalLevel(player) == 2178 && !player.announceMaxLevel) {
			HighscoresHallOfFame.enterToHallOfFame(player, "2079 Total"); // Changing it to 2080 might crash because it
																			// doesn't exist in hall of fame.
