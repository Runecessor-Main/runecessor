...
import game.content.phantasye.skill.artisan.runecrafting.ZMIAltarBankNpc;
	...
	private static boolean firstClickNpcOsrs(Player player, Npc npc) {
		...
		switch (npc.npcType) {
			...
			case 1617:
				...
				if (items.isEmpty()) {
					if (npc.npcType == 1617) {
						if(player.getPetSummoned() == true || player.getPetId() > 0) {
							System.err.println("Server knows this nigga got a pet summoned.");
							player.getPA().sendMessage("You cannot take your pet into Zulrah's instance");
							return false;
						}
			...
			case 5792:
				...
				break;
			// Vote manafger or skilling tokens (eco)
			case 7283:
