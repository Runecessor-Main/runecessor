...
public class NpcDrops {
	...
	/**
	 * The npc id that is representing this npc. When there are multiple npcs
	 * sharing the same drop table, the secondary npcs also have their own
	...
	/**
	 * True if the drop is a pet drop, so when the drop occurs, it can summon it to
	 * follow the player.
	 */
	public ArrayList<Boolean> petDrop = new ArrayList<>();

	public NpcDrops(ArrayList<Integer> npcId, ArrayList<Integer> itemId, ArrayList<Integer> minimumAmount,
			ArrayList<Integer> maximumAmount, ArrayList<Integer> chance, ArrayList<Boolean> announce, int parentNpcId,
			ArrayList<Boolean> petDrop) {
		this.npcId = npcId;
		this.itemId = itemId;
	...
	public static void loadRareDrops() {
		...
		ArrayList<String> arraylist = FileUtility
				.readFileAndSaveEmptyLine(ServerConstants.getDataLocation() + "npc/drops.txt");
		...
		for (int index = 0; index < arraylist.size(); index++) {
			...
			if (line.startsWith("//") && lastLineWasDropRate || index == arraylist.size() - 1) {
				NPC_DROPS[npcId.get(0)] = new NpcDrops(npcId, itemId, minimumAmount, maximumAmount, chance, announce,
						parentNpcId, petDrop);
				lastLineWasDropRate = false;
				...
				if (npcId.size() > 1) {
					for (int i = 1; i < npcId.size(); i++) {
						...
						NPC_DROPS[npcId.get(i)] = new NpcDrops(npcIdLocal, itemId, minimumAmount, maximumAmount, chance,
								announce, npcId.get(0), petDrop);
					}
				}
				npcId = new ArrayList<>();
		...
		for (int index = 0; index < dropsLength; index++) {
			...
			if (chance >= 2 && dropAbove2ChanceReceived >= 1) {
				...
				if (GameType.isOsrsEco()) {
					...
					if (itemId == TINY_CASKET_ID || itemId == SMALL_CASKET_ID || itemId == MEDIUM_CASKET_ID
							|| itemId == LARGE_CASKET_ID) {
						skip = true;
					}
				}
				if (!skip) {
			...
			if (Misc.hasOneOutOf(chance)) {
				...
				if (!dropSimulator) {
					...
					if (petDrop) {
						...
						if (GameType.isOsrs() && ItemAssistant.getItemName(itemId).toLowerCase().contains("ancient")
								&& (npcToKill == 7936 || npcToKill == 7939 || npcToKill == 11124)) {
							player.getPA().sendMessage(
									"Exchange this drop at the Emblem trader who is at lvl 35 wild, in the Revenant caves.");
						}
						
						if (player.getPetId() == 11024 && !(itemId == 13307)) {
							player.getPA().sendMessage("<col=0099ff>Yoshi Pet Collected item to your bank.");
						...
						if (itemId == 13307) {
							player.getPA().sendFilterableMessage(
									"<col=00a000>x" + itemAmount + " blood money has been added to your inventory.");
							ItemAssistant.addItemToInventoryOrDrop(player, 13307, itemAmount);

						}

						itemId = turnNpcDropToNotedForGladiator(player, itemId);
						if (GameType.isOsrsPvp() && itemId == 13307
								&& ItemAssistant.hasItemEquippedSlot(player, 12785, ServerConstants.RING_SLOT)) {
							player.getPA().sendFilterableMessage(
									"<col=00a000>x" + itemAmount + " blood money has been added to your inventory.");
							ItemAssistant.addItemToInventoryOrDrop(player, itemId, itemAmount);
						} else if (!(player.getPetId() == 11024) && !(itemId == 13307)) {
							Server.itemHandler.createGroundItem(player, itemId, dropPosition.getX(),
									dropPosition.getY(), dropPosition.getZ(), itemAmount, false, 0, true, "", "", "",
									"", "giveDropTableDrop " + npcToKill);
						...
						if (announce) {
							...
							if (!player.profilePrivacyOn) {
								Announcement.announce(ServerConstants.GREEN_COL + GameMode.getGameModeName(player)
										+ " has received one " + itemName + " from "
										+ NpcDefinition.getDefinitions()[npcToKill].name + "!");
							}
							RareDropLog.appendRareDrop(player, NpcDefinition.getDefinitions()[npcToKill].name + ": "
	...
	public static void otherDrops(Npc npc, Player player) {
		...
		if (item != 0) {
			...
			Server.itemHandler.createGroundItem(player, item, dropPosition.getX(), dropPosition.getY(),
					dropPosition.getZ(), amount, false, 0, true, "", "", "", "", "otherDrops " + npc.name);
		}
	}
