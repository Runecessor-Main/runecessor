...
public class NpcHandler {
    ...
    /**
     * Last checked, i have 862 npc spawned on server start up. I also need to take
     * in account pets spawned and barrows brothers spawned, Jad minigame, Zombies
     * minigame.
     */
    ...
    /**
     * Npc cannot attack with melee if added here to attack with melee . Npc will
     * hit a max of 1 with melee unless i add it into maxhit
     **/
    ...
    /**
     * Attempts to retrieve the first non-playable character within distance of the
     * given x and y coordinates that is within distance of the given parameter.
     *
     ...
     * @return the first npc with the same id within distance of the given point, or
    ...
    /**
     * Teleport the player next to this npc. Used for Elder Chaos druid for example.
     * <p>
     * Find a spot next to the npc that the npc can walk to, then return these
     * coordinates.
     * <p>
     * if usePlayer is false, then it will find a spot next to the npc that the npc
     * can walk to and return those coordinates.
     *
     * @param xSize TODO
     * @param ySize TODO
     */
    public static int[] teleportPlayerNextToNpc(int playerX, int playerY, boolean usePlayer, int npcX, int npcY,
        ...
        if (((playerX != targetX || playerY != targetY)) && usePlayer
                && Region.isStraightPathUnblocked(npcX, npcY, targetX, targetY, npcHeight, xSize, ySize, false)
                || Region.isStraightPathUnblocked(npcX, npcY, targetX, targetY, npcHeight, xSize, ySize, false)
                && !usePlayer) {
    ...
    /**
     * Attempts to retrieve an open index in the {@link #npcs} array. An open index
     * is one that has a null reference.
     *
     ...
     * @return the new spawned non-playable character or {@link null} if one cannot
     * be created.
     */
    ...
    /**
     * Creates a custom non-playable character implementation if possible for the
     * given type, or null if one cannot be created.
     * <p>
     *
    ...
    public static int getAttackEmote(Npc npc) {
        if (NpcDefinition.getDefinitions()[npc.npcType].name.toLowerCase().contains("dragon")
                && npc.attackType == ServerConstants.DRAGONFIRE_ATTACK) {
            return 81;
        }
        switch (npc.npcType) {
    ...
    public void npcGameTick() {
        ...
        for (int i = 0; i < NPC_INDEX_OPEN_MAXIMUM; i++) {
            ...
            /*
             * Start of Pet. Leave this in process because this for loop can cause a problem
             * when using events for 10 players for example. It's best to have this 'for
             * loop' once every game tick than have 10 players doing it 10 times per game
             * tick.
             */
            ...
            /*
             * if (npc.npcType == 5523) { if (Misc.hasOneOutOf(15)) { npc.
             * forceChat("Donate is mean to keep server alive and keep great update ::donate!"
             * );
             *
             * } }
             */
            /*
             * if (npc.npcType == 547) { if (Misc.hasOneOutOf(15)) { npc.
             * forceChat("Hello Runecessor mebmers go above  wild 18 to kill Boss For Reward event key!"
             * );
             *
             * } if (Misc.hasOneOutOf(30)) { npc.
             * forceChat("Use 20k On chest By the side me to reward 1 Mystery Box and Spin it Good Luck!"
             * );
             *
             * } }
             */
            /*
             * if (npc.npcType == 3902) { if (Misc.hasOneOutOf(15)) {
             * npc.forceChat("Check Out my Easter Event Today Click me!");
             *
             * } if (Misc.hasOneOutOf(30)) {
             * npc.forceChat("Use 2k Easter Key On chest By the side me!");
             *
             * } }
             */
            ...
            if ((npc.getKillerId() > 0 || npc.underAttack || npc.getFollowingType() != null) && !npc.isWalkingHome()
                    && npc.getNpcPetOwnerId() == 0) {
                if (!npc.isDead()) {
            ...
            if ((!npc.underAttack || npc.isWalkingHome()) && npc.randomWalk && !npc.isDead()
                    && System.currentTimeMillis() - npc.timeAttackedAPlayer > 10000 && !npc.summoned
                    && !npc.neverRandomWalks()) {
                npc.facePlayer(0);
                npc.setKillerId(0);
...
//                    DegradingManager.degradeFromKills(player);
    ...
    /**
     * @return True, if the npc can switch targets when player is in a multi-zone.
     */
...
//            NpcDrops.otherDrops(npc, player);
     ...
     *
     * @param npcIndex       the index of the npc we're trying to have follow the
     *                       second.
    ...
    public static int applyDamageOnPlayerFromNPC(int playerIndex, Npc npc, int attackType, boolean damageQueue,
        ...
        if (npc.npcType == 11124 && ItemAssistant.hasItemEquippedSlot(player, 21816, ServerConstants.HAND_SLOT)) {

            return 0;
        }
        if (npc.npcType == 7936 && ItemAssistant.hasItemEquippedSlot(player, 21816, ServerConstants.HAND_SLOT)) {

            return 0;
        }
        if (npc.npcType == 7939 && ItemAssistant.hasItemEquippedSlot(player, 21816, ServerConstants.HAND_SLOT)) {
...
//			damage = PrayerManager.getBook(player).getDamageEffects(player, damage, attackType);
    ...
    public static int calculateNpcMeleeDamage(Npc npc, Player player, int fixedDamage, int maximumDamage) {
        ...
        if (10 + Misc.random(MeleeFormula.getInvisibleMeleeDefenceAdvantage(player)) > Misc
                .random(NpcDefinition.getDefinitions()[npc.npcType].attack)) {
            damage = 0;
        }
        if (player.prayerActive[ServerConstants.PROTECT_FROM_MELEE]) {
    ...
    public static void kill(int npcType, int height) {
        Arrays.stream(npcs).filter(Objects::nonNull).filter(n -> n.npcType == npcType && n.getHeight() == height)
                .forEach(npc -> npc.isDead());
    }
}
